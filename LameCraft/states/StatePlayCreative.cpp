...
void
StatePlayCreative::InitParametric(bool makeTrees, bool makeWater, bool makeCaves, unsigned int seed_1, int worldType,
                                  char gameMode) {
    //set render manager instance
    mRender = RenderManager::InstancePtr();
    mSystemMgr = SystemManager::Instance();
...
void StatePlayCreative::LoadTextures() {
    ...
    barSprite = new Sprite(TextureHelper::Instance()->GetTexture(TextureHelper::Utils), 0, 0, 182 * utilsSize / 182,
                           22 * utilsSize / 182);
    barSprite->SetPosition(240, 250);
    barSprite->Scale(utilScale, utilScale);

    selectSprite = new Sprite(TextureHelper::Instance()->GetTexture(TextureHelper::Utils), 0, 22 * utilsSize / 182,
                              24 * utilsSize / 182, 24 * utilsSize / 182);
    ...
    sliderLightedSprite = new Sprite(TextureHelper::Instance()->GetTexture(TextureHelper::invSlider), 12, 0, 12,
                                     15);//200
    sliderLightedSprite->SetPosition(240, 136);
    sliderLightedSprite->Scale(invGuiScale, invGuiScale);

    float invArrowSize = TextureManager::Instance()->getWidth(
    ...
    if (enableMainCraftBody) {
        switch (result) {
            ...
            case 312: // spruce
                ...
                // torch
            case 553:
                if ((craftSlotId[0] == 277 && craftSlotId[2] == 276) ||
                    (craftSlotId[1] == 277 && craftSlotId[3] == 276)) {
                    craftItemId = 308;
                    craftItemSt = 1;
                    craftItemAm = 4;
                }
            ...
            if (sleepTime == 0.0f) {
                ...
                if (keyHold(InputHelper::Instance()->getButtonToAction(12))) {
                    ...
                    if (headInWater || headInLava ||
                        mWorld->BlockAtPoint(Vector3(playerPosition.x, playerPosition.y - 0.65f, playerPosition.z)) ==
                        WaterBlock::getID() ||
                        mWorld->BlockAtPoint(Vector3(playerPosition.x, playerPosition.y - 0.65f, playerPosition.z)) ==
                        Lava::getID())    //Legit swimming
                    {
                        if (walkingOnGround) {
                            jumping = true;
                        } else {
                    ...
                    if ((!headInWater || !headInLava) && !walkingOnGround &&
                        (footInWater || footInLava))    //Your above he water, so Jump out
                    {
                ...
                if (keyPressed(InputHelper::Instance()->getButtonToAction(14))) {
                    if (mWorld->invId[27 + barPosition] > 0) {
                        ...
                        if (mWorld->invSt[27 + barPosition] == true) {
                            ...
                            mWorld->DropThisNoAlign(mWorld->invId[27 + barPosition], mWorld->invAm[27 + barPosition],
                                                    mWorld->invSt[27 + barPosition],
                                                    Vector3(fppCam->m_vPosition.x, fppCam->m_vPosition.y - 0.25f,
                                                            fppCam->m_vPosition.z), dropVelocity);
                ...
                for (float i = 0; i < 5.25f; i += 0.15f) {
                    ...
                    if (mWorld->BlockEditable(testPos.x, testPos.y, testPos.z) && testBox.contains(testPos)) {
                        ...
                        if (selectedBlock == CraftingTable::getID()) // open crafting table menu
                        {
                            ...
                            upEn == true ? selectInvSprite->SetPosition(204 + (invXPosition * 36),
                                                                        26 + (invYPosition * 36))
                                         : selectInvSprite->SetPosition(96 + (invXPosition * 36),
                                                                        138 + (invYPosition * 36));
                                ...
                                if (!blockBox.intersect(playerBox)) {
                                    switch (mWorld->invId[27 + barPosition]) {
                                        ...
                                        case 108 :
                                            ...
                                            break;
                                        case 157 :
                                            mWorld->GetBlock(testPos.x, testPos.y, testPos.z) = BirchPlanks::getID();
                                            break;
                                        case 158 :
                                            mWorld->GetBlock(testPos.x, testPos.y, testPos.z) = SprucePlanks::getID();
                                            break;
                                    }
                        ...
                        if (selectedBlock == ChestBlock1::getID() || selectedBlock == ChestBlock2::getID() ||
                            ...
                            if (chestId != -1) {
                                ...
                                upEn == true ? selectInvSprite->SetPosition(96 + (invXPosition * 36),
                                                                            26 + (invYPosition * 36))
                                             : selectInvSprite->SetPosition(96 + (invXPosition * 36),
                                                                            138 + (invYPosition * 36));
                        ...
                        if (selectedBlock == FurnaceOff::getID() || selectedBlock == FurnaceOn::getID()) {
                            ...
                            if (furnaceId != -1) {
                                ...
                                upEn == true ? selectInvSprite->SetPosition(204, 28 + (invYPosition * 72))
                                             : selectInvSprite->SetPosition(96 + (invXPosition * 36),
                                                                            138 + (invYPosition * 36));
                        ...
                        if (distance != -1.0f) {
                            ...
                            if (newPosBlockId == Lava::getID() || newPosBlockId == 4 || newPosBlockId == 0 ||
                                newPosBlockId == 93 ||
                                (newPosBlockId >= WaterBlock1::getID() && newPosBlockId <= WaterBlock4::getID())) {
                                    ...
                                    if (mWorld->CanPutBlockHere(testPos2.x, testPos2.y, testPos2.z,
                                            ...
                                            WhiteWoolCarpet::getID()) // if carpet in our hands
                                        {
                                            if (mWorld->BlockSpecial(testPos2.x, testPos2.y - 1, testPos2.z) != 2) {
                                                if (mWorld->BlockSpecial(testPos2.x, testPos2.y - 1, testPos2.z) != 0 ||
                                        ...
                                        if (mWorld->invId[27 + barPosition] < 250) // if block in our hands
                                        {
                                            ...
                                            {
                                                NoteBlockEntity *NewNoteBlockEntity = new NoteBlockEntity(testPos2.x,
                                                                                                          testPos2.y,
                                                                                                          testPos2.z,
                                                                                                          12);
                                            ...
                                            if (mWorld->invId[27 + barPosition] != -1) //multi-texturing blocks
                                            {
                                                ...
                                                switch (mWorld->invId[27 + barPosition]) {
                                                    ...
                                                    default:
                                                        mWorld->GetBlock(testPos2.x, testPos2.y,
                                                                         testPos2.z) = mWorld->invId[27 + barPosition];
                                                        break;
                                                }
                                        ...
                                        } else //if item in our hand
                                        {
                                            ...
                                            if (mWorld->invId[27 + barPosition] ==
                                                IronBarsItem::getID()) // if it is iron bars
                                            {
                                                mWorld->GetBlock(testPos2.x, testPos2.y,
                                            ...
                                            if (mWorld->invId[27 + barPosition] >= 294 &&
                                                mWorld->invId[27 + barPosition] <= 296) // if it is flowers or saplings
                                            {
                                                ...
                                                {
                                                    mWorld->GetBlock(testPos2.x, testPos2.y, testPos2.z) =
                                                            119 + (mWorld->invId[27 + barPosition] - 294);
                                                    fppCam->needUpdate = true;
                                                    int chunkTarget = mWorld->getChunkId(testPos2);
                                                    mWorld->RebuildTransparentMeshChunk(chunkTarget);
                                            ...
                                            if (mWorld->invId[27 + barPosition] ==
                                                ItemFrameItem::getID()) // if it is item frame
                                            {
                                                bool placed = false;
                                            ...
                                            if (mWorld->invId[27 + barPosition] == 293) // if it is cane item
                                            {
                                                ...
                                                {
                                                    if (mWorld->GetBlock(testPos2.x - 1, testPos2.y - 1, testPos2.z) ==
                                                        4 ||
                                                        mWorld->GetBlock(testPos2.x + 1, testPos2.y - 1, testPos2.z) ==
                                                        4 ||
                                                        mWorld->GetBlock(testPos2.x, testPos2.y - 1, testPos2.z + 1) ==
                                                        4 ||
                                                        mWorld->GetBlock(testPos2.x, testPos2.y - 1, testPos2.z - 1) ==
                                                        4) {
                                        ...
                                        if (wasLight == true) {
                                            ...
                                            if (!wasUpdated) {
                                                ...
                                                if (mWorld->BlockTransparent(testPos2.x, testPos2.y, testPos2.z) ||
                                                    mWorld->blockTypes[mWorld->GetBlock(testPos2.x, testPos2.y,
                                                                                        testPos2.z)].blockModel == 2) {
                                                    mWorld->rebuildNearestChunks(chunkTarget, testPos2);
                                                }
                                            }
                                        }
                ...
                for (float i = 0; i < 5.25f; i += 0.15f) {
                    ...
                    if (mWorld->BlockEditable(testPos2.x, testPos2.y, testPos2.z) && testBox.contains(testPos2)) {
                            ...
                            if (upper_block != 0) {
                                ...
                                for (int checkY = testPos1.y + 1; checkY <= testPos1.y + 5; checkY++) {
                                    ...
                                    if (mWorld->GetBlock(testPos2.x, testPos2.y, testPos2.z) != CaneBlock::getID() &&
                                        mWorld->GetBlock(testPos2.x, testPos2.y, testPos2.z) != CactusBlock::getID()) {
                                        if (mWorld->ChunksEqual(testPos1, testPos2) == false) {
                                            int chunkTarget = mWorld->getChunkId(testPos2);
                                            mWorld->RebuildTransparentMeshChunk(chunkTarget);
                                        }
                        ...
                        if (mParticles.size() <= 52) {
                            ...
                            {
                                Particle *Smoke2 = new Particle(mWorld, "block crack", Vector3((int) testPos1.x + 0.5f +
                                                                                               (-0.3 +
                                                                                                (rand() % 7) * 0.1f),
                                                                                               (int) testPos1.y + 0.5f +
                                                                                               (-0.2 +
                ...
                if (upEn == 0) {
                    if (invEn == false) {
                        ...
                        if (onDestroySlot) {
                            ...
                            if (mWorld->mId == -1) {
                                if (creativePage * 27 + invYPosition * 9 + invXPosition <
                                    inventoryItems.size()) // creativePage*53 + invYPosition*9 + invXPosition - выбранный слот в инветаре с учетом страницы
                                {
                                    ...
                                    if (inventoryItems[creativePage * 27 + invYPosition * 9 + invXPosition] >= 250) {
                                        if (mWorld->DurabilityPointsItem(
                                                inventoryItems[creativePage * 27 + invYPosition * 9 + invXPosition]) !=
                                            -1) {
                                            mWorld->mAm = mWorld->DurabilityPointsItem(
                                                    inventoryItems[creativePage * 27 + invYPosition * 9 +
                                                                   invXPosition]);
                                            mWorld->mSt = 0;
                                        } else {
                                            if (mWorld->StackableItem(
                                                    inventoryItems[creativePage * 27 + invYPosition * 9 +
                                                                   invXPosition])) {
                                                mWorld->mAm = 64;
                                                mWorld->mSt = true;
                        ...
                        if (mWorld->mId != -1) {
                            ...
                            if (UseChest->chestSlotId[invYPosition * 9 + invXPosition] != -1 &&
                                ...
                                if (UseChest->chestSlotAm[invYPosition * 9 + invXPosition] > 1) {
                                    mWorld->mId = UseChest->chestSlotId[invYPosition * 9 + invXPosition];
                                    mWorld->mAm = ceilf(
                                            (float) (UseChest->chestSlotAm[invYPosition * 9 + invXPosition] / 2.0f));
                                    mWorld->mSt = UseChest->chestSlotSt[invYPosition * 9 + invXPosition];
...
void StatePlayCreative::Draw(StateManager *sManager) {
    ...
    */
    ...
    if (mWorld->mainOptions.guiDrawing == 1) {
        ...
        if (mWorld->invId[27 + barPosition] == -1) {
            TextureManager::Instance()->SetTextureModeulate(
                    TextureHelper::Instance()->GetTexture(TextureHelper::Steve));
            sceGumPushMatrix();
    ...
    if ((invEn == false && craft3xEn == false && chestEn == false && furnaceEn == false && menuState == 0 &&
         mWorld->mainOptions.guiDrawing == 1) || makeScreen == true) {
        barSprite->Draw();
        if (menuState != 1) {
    ...
    if ((invEn == false && craft3xEn == false && chestEn == false && menuState == 0 &&
         mWorld->mainOptions.guiDrawing == 1 && furnaceEn == false) || makeScreen == true) {
        for (int k = 0; k <= 8; k++) {
            ...
            for (int j = 0; j <= 8; j++) {
                if (creativePage * 27 + i * 9 + j < inventoryItems.size()) {
                    if (inventoryItems[creativePage * 27 + i * 9 + j] != -1) {
                        ...
                        if (inventoryItems[creativePage * 27 + i * 9 + j] >= 250) {
                            mWorld->ItemHaveTerrainTexture(inventoryItems[creativePage * 27 + i * 9 + j])
                            ? TextureManager::Instance()->SetTextureModeulate(texture)
                            : TextureManager::Instance()->SetTextureModeulate(barItems);
    ...
    if (invEn == false && craft3xEn == false && chestEn == false && furnaceEn == false && menuState == 0) {
        if (tickShowFlymodeMessage > 0.0f) {
            if (tickShowFlymodeMessage > 1.0f) {
                ...
                mRender->SetFontStyle(0.5f, GU_COLOR(1, 1, 1, tickShowFlymodeMessage),
                                      GU_COLOR(0.15f, 0.15f, 0.15f, tickShowSlotName), 0x00000200);
            }
            if (language == ENGLISH) {
                canFly == true ? mRender->DebugPrint(240, 30, "Fly mode : on") : mRender->DebugPrint(240, 30,
                                                                                                     "Fly mode : off");
            }
            if (language == RUSSIAN) {
                canFly == true ? mRender->DebugPrint(240, 30, "Rejim poleta : vkl") : mRender->DebugPrint(240, 30,
                                                                                                          "Rejim poleta : v@kl");
            }
        }
        ...
        if (menuOptions) {
            ...
            if (language == ENGLISH) {
                ...
                optionsMenuPos == 10 ? RenderManager::InstancePtr()->SetFontStyle(default_size, GU_COLOR(1, 1, 0, 1), 0,
                                     ...
                                     : RenderManager::InstancePtr()->SetFontStyle(default_size, GU_COLOR(1, 1, 1, 1), 0,
                                                                                  0x00004000 | 0x00000200);
                mWorld->mainOptions.freezeDayTime == true ? mRender->DebugPrint(370, 109, "Freeze Day Time: ON")
                                                          : mRender->DebugPrint(370, 109, "Freeze Day Time: OFF");
            ...
            if (language == RUSSIAN) {
                ...
                optionsMenuPos == 10 ? RenderManager::InstancePtr()->SetFontStyle(default_size, GU_COLOR(1, 1, 0, 1), 0,
                                     ...
                                     : RenderManager::InstancePtr()->SetFontStyle(default_size, GU_COLOR(1, 1, 1, 1), 0,
                                                                                  0x00004000 | 0x00000200);
                mWorld->mainOptions.freezeDayTime == true ? mRender->DebugPrint(370, 109, "Zamorozit$ vrem^ : Vkl")
                                                          : mRender->DebugPrint(370, 109, "Zamorozit$ vrem^ : V@kl");
