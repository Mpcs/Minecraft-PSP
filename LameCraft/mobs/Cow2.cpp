...
void Cow::BoxCollision() {
    for (int x = 0; x < world->mCows.size(); x++) {
        ...
        if (TestCow2->number != number && (TestCow2->position.x != position.x || TestCow2->position.z != position.z ||
                                           TestCow2->position.y != position.y)) {
            if (TestCow2->bBox.intersect(bBox)) {
                float angle1 = ATAN2_fast(position.x - TestCow2->position.x,
                                          position.z - TestCow2->position.z); // super secret function
