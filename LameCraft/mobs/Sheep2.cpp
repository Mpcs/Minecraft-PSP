...
void Sheep::FetchFurColor() {
    switch (color) {
        case 0 :
            furColor = Vector3(0.19f, 0.16f, 0.16f);
            break;  // black
        case 1 :
            furColor = Vector3(0.38f, 0.38f, 0.38f);
            break;  // gray
        case 2 :
            furColor = Vector3(0.73f, 0.2f, 0.2f);
            break; // red
        case 3 :
            furColor = Vector3(1, 0.67f, 0.67f);
            break; // pink
        case 4 :
            furColor = Vector3(0.19f, 0.38f, 0.1f);
            break; // dark green
        case 5 :
            furColor = Vector3(0.19f, 0.77f, 0.13f);
            break; // green
        case 6 :
            furColor = Vector3(0.35f * 1.1f, 0.20f * 1.1f, 0.13f * 1.1f);
            break; // brown
        case 7 :
            furColor = Vector3(0.73f, 0.73f, 0.1f);
            break; // yellow
        case 8 :
            furColor = Vector3(0.24f, 0.24f, 0.68f);
            break; // blue
        case 9 :
            furColor = Vector3(0.45f, 0.52f, 0.9f);
            break; // light blue
        case 10 :
            furColor = Vector3(0.63f, 0.29f, 0.73f);
            break; // purple
        case 11 :
            furColor = Vector3(0.73f, 0.42f, 0.73f);
            break; // magneta
        case 12 :
            furColor = Vector3(0.8f, 0.4f, 0.1f);
            break; // orange
        case 13 :
            furColor = Vector3(0.73f, 0.73f, 0.73f);
            break; // light gray
        case 14 :
            furColor = Vector3(1, 1, 1);
            break;
        default :
            furColor = Vector3(1, 1, 1);
            break;
...
void Sheep::BoxCollision() {
    for (int x = 0; x < world->mSheeps.size(); x++) {
        ...
        if (TestSheep2->number != number &&
            (TestSheep2->position.x != position.x || TestSheep2->position.z != position.z ||
             TestSheep2->position.y != position.y)) {
            if (TestSheep2->bBox.intersect(bBox)) {
                float angle1 = ATAN2_fast(position.x - TestSheep2->position.x,
                                          position.z - TestSheep2->position.z); // super secret function
...
void Sheep::Update(CraftWorld *myWorld, Vector3 playerPos, float dt) {
    ...
    BoundingBox playerHitBox = BoundingBox(Vector3(playerPos.x - 0.35f, playerPos.y - 1.65f, playerPos.z - 0.35f),
                                           Vector3(playerPos.x + 0.35f, playerPos.y + 0.2f, playerPos.z + 0.35f));
        ...
        if (fastDistanceToPlayer < world->mainOptions.horizontalViewDistance * 14 + 5) {
            ...
            if (dt < 0.1f) {
                ...
                if (kicked == true) {
                    ...
                    TryToMoveWithoutJump(Vector3(sinf(mainAngle2 + PI), 0.0f, cosf(mainAngle2 + PI)),
                                         (SPD + 0.18f) * 9.2f * (damagedTimer * damagedTimer) * dt);
                ...
                if (world->SolidAtPointForEntity(footPosition, colliderD) ||
                    world->SolidAtPointForEntity(headPosition, colliderU)) {
...
void Sheep::Render(Frustum &camFrustum, float dt) {
    if (toDestroy == false) {
        if (fastDistanceToPlayer < world->mainOptions.horizontalViewDistance * 14 &&
            camFrustum.BoxInFrustum(bBox) == Frustum::Intersects) {
            sceGumPushMatrix();
            ...
            if (sheared) {
                ...
                if ((damaged == true && damagedTimer > 0.25f) || killed == true) {
                    sceGuColor(GU_COLOR(myLight * furColor.x * 0.8f, myLight * furColor.y * 0.1,
                                        myLight * furColor.z * 0.1, 1.0f));
                } else {
                    sceGuColor(GU_COLOR(myLight * furColor.x, myLight * furColor.y, myLight * furColor.z, 1.0f));
                }
