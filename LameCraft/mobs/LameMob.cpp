...
float LameMob::DistanceToPlayer() {
    return abs(relPlayerPos.x - position.x) + abs(relPlayerPos.z - position.z) +
           abs(relPlayerPos.y - 0.7 - position.y) * 0.89f;
}

unsigned int LameMob::FastDistanceToPlayer() {
    return world->FastDistance2d(abs(position.x - world->playerPos.x), abs(position.z - world->playerPos.z)) +
           abs(relPlayerPos.y - (position.y - 0.5)) * 0.7f;
}

float LameMob::PreciseDistanceToPlayer() /// REAL DISTANCE
{
    return sqrt((relPlayerPos.x - position.x) * (relPlayerPos.x - position.x) +
                (relPlayerPos.z - position.z) * (relPlayerPos.z - position.z));
}
...
void LameMob::TryToMove(Vector3 movePosition1, float speed) {
    ...
    if (!world->SolidAtPointForEntity(movePosition, colliderD) &&
        !world->SolidAtPointForEntity(movePositionHead, colliderU) &&
        !world->SolidAtPointForEntity(movePositionFoot, colliderD)) {
        Vector3 collisionPosition =
                position + Vector3(sinf(mainAngle) * (speed + 0.25), 0.0f, cosf(mainAngle) * (speed + 0.25));
...
void LameMob::UpdateBBox() {
    bBox = BoundingBox(
            Vector3(position.x - colliderD.x * 1.45, position.y - colliderD.y * 1.3, position.z - colliderD.z * 1.45),
            Vector3(position.x + colliderU.x * 1.45, position.y + colliderU.y * 1.3, position.z + colliderU.z * 1.45));
}
