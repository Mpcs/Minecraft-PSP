...
void Zombie::BoxCollision() {
    for (int x = 0; x < world->mZombies.size(); x++) {
        ...
        if (TestZombie2->number != number &&
            (TestZombie2->position.x != position.x || TestZombie2->position.z != position.z ||
             TestZombie2->position.y != position.y)) {
            if (TestZombie2->bBox.intersect(bBox)) {
                float angle1 = ATAN2_fast(position.x - TestZombie2->position.x,
                                          position.z - TestZombie2->position.z); // super secret function
        ...
        if (dt < 0.1f) {
            ...
            if (sunTimer > 1.5f) {
                if ((world->worldDayTime > 6 && world->worldDayTime < 21 &&
                     world->GetShadowLevel(position.x, position.y - 0.2f, position.z) == 0) &&
                    world->worldWeather->GetWeatherType() == 0)//clear weather, sun, zombie under clear sky
                {
                    HP -= 2.0f;
                    damaged = true;
                    damagedTimer = 0.9f;
            ...
            if (dt < 0.1f) {
                ...
                if (nearToPlayer == true) {
                    ...
                    if (kickTimer >= 1.0f) {
                        ...
                        if (world->BlockSolid(position.x + checkPos.x, position.y + checkPos.y,
                                              position.z + checkPos.z) == false &&
                            world->BlockSolid(position.x + checkPos2.x, position.y + checkPos2.y,
