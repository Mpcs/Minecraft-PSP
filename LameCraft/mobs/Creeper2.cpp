...
void Creeper::BoxCollision() {
    for (int x = 0; x < world->mCreepers.size(); x++) {
        ...
        if (TestCreeper2->number != number &&
            (TestCreeper2->position.x != position.x || TestCreeper2->position.z != position.z ||
             TestCreeper2->position.y != position.y)) {
            if (TestCreeper2->bBox.intersect(bBox)) {
                float angle1 = ATAN2_fast(position.x - TestCreeper2->position.x,
                                          position.z - TestCreeper2->position.z); // super secret function
    ...
    if (toDestroy == false && exploded == false) {
        ...
        if (fastDistanceToPlayer < world->mainOptions.horizontalViewDistance * 14 + 5) {
            ...
            if (dt < 0.1f) {
                ...
                float dist = world->FastDistance2d(abs(position.x - world->playerPos.x) * 10,
                                                   abs(position.z - world->playerPos.z) * 10) / 10.0f +
                             abs(position.y - (world->playerPos.y - 0.7)) / 2.0f;
                if (startExplosion) {
                    if (dist < 6.21f) {
                        ...
                        if (timeBeforeExplosion < 0.0f) {
                            if (dist < 4.6f) {
                                ...
                                world->kickedAngle = ATAN2_fast(position.x - world->playerPos.x,
                                                                position.z - world->playerPos.z);
                                world->kickedBy = 2;
                            ...
                            world->AddChunkToFastUpdate(
                                    world->getChunkId(Vector3(position.x + 4, position.y + 4, position.z + 4)));
                            world->AddChunkToFastUpdate(
                                    world->getChunkId(Vector3(position.x - 4, position.y + 4, position.z + 4)));
                            world->AddChunkToFastUpdate(
                                    world->getChunkId(Vector3(position.x - 4, position.y + 4, position.z - 4)));
                ...
                if (killed == false) {
                    if (fastDistanceToPlayer < 11 && world->health > 0 && boredom == false) // it sees you
                    {
                        ...
                        if (dist > 2.3f) {
                            ...
                            if (world->BlockSolid(position.x + (world->playerPos.x - position.x) / 2.0f,
                                                  position.y + 0.4f,
                                                  position.z + (world->playerPos.z - position.z) / 2.0f) == false) {
                                moving = false;
                                boredomTime = 0.0f;
                ...
                if (kicked == true) {
                    ...
                    mainAngle = ATAN2_fast(position.x - relPlayerPos.x, position.z - relPlayerPos.z) +
                                PI; // super secret function
                    ...
                    TryToMoveWithoutJump(Vector3(sinf(angle + PI), 0.0f, cosf(angle + PI)),
