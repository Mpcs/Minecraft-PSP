...
CraftWorld::CraftWorld() {
    ...
    playerZoneSize = Vector3(14 * mainOptions.horizontalViewDistance, 15 * mainOptions.verticalViewDistance, 14 *
                                                                                                             mainOptions.horizontalViewDistance); // Зона прорисовки чанков (каждый параметр должен быть кратен размеру чанка)
        ...
        for (x = 0; x < WORLD_SIZE; ++x) {
            ...
            for (y = WORLD_HEIGHT - 1; y >= 0; --y) {
                ...
                if (level < 15) {
                    ...
                    if (GetBlock(x, y, z) == LeavesBlock::getID() || GetBlock(x, y, z) == BirchLeaves::getID() ||
                        GetBlock(x, y, z) == SpruceLeaves::getID()) {
                        if (level == 0) {
                            level += 2;
                        }
                    }
...
void CraftWorld::CalculateBlocksLightAtPos(const int xx, const int yy, const int zz,
                                           int iteration) // dont finished, not used
{
    /*  int level = 0;
    if(iteration == 1)
    {
...
void CraftWorld::initPutBlocksLight(const int xx, const int yy, const int zz) // Тень под блоками
{
    ...
    for (y = yy; y >= 0; --y) {
        SetShadowLevel(x, y, z, level);
        if (level < 15) {
            if (GetBlockNoCheck(x, y, z) != 0 && BlockRefraction(x, y, z) != 0 && BlockRefraction(x, y, z) != 2 ||
                GetBlockNoCheck(x, y, z) == WaterBlock::getID()) {
                level += 1;
            }
...
void
CraftWorld::setTextureSize(int texture, int chunk, int animTexture, int animChunk, int itemTexture, int itemChunk) {
    texutreSize = texture;
    textureChunk = chunk;
...
void CraftWorld::DropThis(int id, Vector3 position) {
    if (id != -1) {
        dropNum++;
        Drop *NewDrop = new Drop(Vector3((int) position.x + 0.5f, (int) position.y + 0.51f, (int) position.z + 0.5f),
                                 id, 1, true, dropNum);
        NewDrop->myCrft = this;
...
void CraftWorld::DestroyItemFrameEntity(Vector3 position) {
    ...
    if (id != -1) {
        ...
        if (DestroyIF->GetAmount() != -1) {
            DropThis(DestroyIF->GetItem(), DestroyIF->GetAmount(), StackableItem(DestroyIF->GetItem()),
                     Vector3((int) position.x + 0.5f, (int) position.y + 0.5f, (int) position.z + 0.5f));
        } else {
            if (DestroyIF->GetItem() >= 250) {
                DropThis(DestroyIF->GetItem(), 1, StackableItem(DestroyIF->GetItem()),
                         Vector3((int) position.x + 0.5f, (int) position.y + 0.5f, (int) position.z + 0.5f));
            } else {
                DropThis(DestroyIF->GetItem(), 1, true,
                         Vector3((int) position.x + 0.5f, (int) position.y + 0.5f, (int) position.z + 0.5f));
            }
        }
        DropThis(ItemFrameItem::getID(), 1, true,
                 Vector3((int) position.x + 0.5f, (int) position.y + 0.5f, (int) position.z + 0.5f));
...
void
CraftWorld::RemoveLigtSourceAtPosition(const int x, const int y, const int z, int blockID) // Выпилить источник света
{
...
bool CraftWorld::CanPutBlockHere(const int x, const int y, const int z, int blockID) {
    if (BlockSolid(x - 1, y, z) == true || BlockSolid(x + 1, y, z) == true || BlockSolid(x, y, z - 1) == true ||
        BlockSolid(x, y, z + 1) == true || BlockSolid(x, y - 1, z) == true ||
        (BlockSolid(x, y + 1, z) == true && blockID != TorchItem::getID())) {
        return true;
    }
    return false;
}
...
bool CraftWorld::BlockTransparentOrSpecial(const int x, const int y, const int z) {
    ...
    if (blockTypes[m_Blocks[x + y * WORLD_SIZE + z * WORLD_SIZE * WORLD_HEIGHT]].transparent ||
        blockTypes[m_Blocks[x + y * WORLD_SIZE + z * WORLD_SIZE * WORLD_HEIGHT]].blockModel > 0)
        return true;
...
bool CraftWorld::BlockTransparentOrLightSource(const int x, const int y, const int z) {
    if (x >= 0 && x < WORLD_SIZE && z >= 0 && z < WORLD_SIZE && y >= 0 && y < WORLD_HEIGHT) {
        if (blockTypes[m_Blocks[x + y * WORLD_SIZE + z * WORLD_SIZE * WORLD_HEIGHT]].transparent ||
            blockTypes[m_Blocks[x + y * WORLD_SIZE + z * WORLD_SIZE * WORLD_HEIGHT]].lightSource ||
            blockTypes[m_Blocks[x + y * WORLD_SIZE + z * WORLD_SIZE * WORLD_HEIGHT]].refraction == 2)
            return true;
...
int CraftWorld::DurabilityPointsItem(int id) {
    ...
    if (item->hasFeature(FeatureType::durable)) {
        ...
        return durabilityFeature->getMaxDurability();
    }
...
void
CraftWorld::GetSpecialBlockVerts(int i, BaseBlock *blockType) // Строим меш инвентарных блоков, имеющих особую модель
{
    iVertexLeftRight = 0;
    pointsLeftRight = 0;
    iVertexBackFront = 0;
    ...
    if (blockType->blockModel == 5) //not common model
    {
        if (i == 6 || i == 134 || i == 135 || i == 136 || (i >= 87 && i <= 90)) {
            BuildGUIBlockPlane(blockType, 1, Vector3(0, 0, 16), Vector3(0, 16, 16), Vector3(0, 16, 0),
                               Vector3(0, 0, 0));
            BuildGUIBlockPlane(blockType, 2, Vector3(16, 0, 0), Vector3(16, 16, 0), Vector3(16, 16, 16),
                               Vector3(16, 0, 16));
            BuildGUIBlockPlane(blockType, 3, Vector3(0, 0, 0), Vector3(16, 0, 0), Vector3(16, 0, 16),
                               Vector3(0, 0, 16));
            BuildGUIBlockPlane(blockType, 4, Vector3(0, 16, 16), Vector3(16, 16, 16), Vector3(16, 16, 0),
                               Vector3(0, 16, 0));
            BuildGUIBlockPlane(blockType, 5, Vector3(0, 16, 0), Vector3(16, 16, 0), Vector3(16, 0, 0),
                               Vector3(0, 0, 0));
            BuildGUIBlockPlane(blockType, 6, Vector3(0, 0, 16), Vector3(16, 0, 16), Vector3(16, 16, 16),
                               Vector3(0, 16, 16));
        }
        ...
        if (i >= BlackWoolCarpet::getID() && i <= WhiteWoolCarpet::getID()) {
            ...
            BuildGUIBlockPlane(blockType, 2, Vector3(16, 0, 0), Vector3(16, 1, 0), Vector3(16, 1, 16),
                               Vector3(16, 0, 16));
            BuildGUIBlockPlane(blockType, 3, Vector3(0, 0, 0), Vector3(16, 0, 0), Vector3(16, 0, 16),
                               Vector3(0, 0, 16));
            BuildGUIBlockPlane(blockType, 4, Vector3(0, 1, 16), Vector3(16, 1, 16), Vector3(16, 1, 0),
                               Vector3(0, 1, 0));
            BuildGUIBlockPlane(blockType, 5, Vector3(0, 1, 0), Vector3(16, 1, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
            BuildGUIBlockPlane(blockType, 6, Vector3(0, 0, 16), Vector3(16, 0, 16), Vector3(16, 1, 16),
                               Vector3(0, 1, 16));
        }
        ...
        if (i == TrapdoorClosed1::getID()) {
            ...
            BuildGUIBlockPlane(blockType, 2, Vector3(16, 0, 0), Vector3(16, 3, 0), Vector3(16, 3, 16),
                               Vector3(16, 0, 16));
            BuildGUIBlockPlane(blockType, 3, Vector3(0, 0, 0), Vector3(16, 0, 0), Vector3(16, 0, 16),
                               Vector3(0, 0, 16));
            BuildGUIBlockPlane(blockType, 4, Vector3(0, 3, 16), Vector3(16, 3, 16), Vector3(16, 3, 0),
                               Vector3(0, 3, 0));
            BuildGUIBlockPlane(blockType, 5, Vector3(0, 3, 0), Vector3(16, 3, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
            BuildGUIBlockPlane(blockType, 6, Vector3(0, 0, 16), Vector3(16, 0, 16), Vector3(16, 3, 16),
                               Vector3(0, 3, 16));
        }

        if (i == Snow2::getID()) {
            BuildGUIBlockPlane(blockType, 1, Vector3(0, 0, 16), Vector3(0, 2, 16), Vector3(0, 2, 0), Vector3(0, 0, 0));
            BuildGUIBlockPlane(blockType, 2, Vector3(16, 0, 0), Vector3(16, 2, 0), Vector3(16, 2, 16),
                               Vector3(16, 0, 16));
            BuildGUIBlockPlane(blockType, 3, Vector3(0, 0, 0), Vector3(16, 0, 0), Vector3(16, 0, 16),
                               Vector3(0, 0, 16));
            BuildGUIBlockPlane(blockType, 4, Vector3(0, 2, 16), Vector3(16, 2, 16), Vector3(16, 2, 0),
                               Vector3(0, 2, 0));
            BuildGUIBlockPlane(blockType, 5, Vector3(0, 2, 0), Vector3(16, 2, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
            BuildGUIBlockPlane(blockType, 6, Vector3(0, 0, 16), Vector3(16, 0, 16), Vector3(16, 2, 16),
                               Vector3(0, 2, 16));
        }
        ...
        if (i == Torch::getID()) {
            ...
            BuildGUIBlockPlane(blockType, 4, Vector3(7, 10, 9), Vector3(9, 10, 9), Vector3(9, 10, 7),
                               Vector3(7, 10, 7));
        ...
        if (i == CactusBlock::getID()) {
            BuildGUIBlockPlane(blockType, 1, Vector3(1, 0, 16), Vector3(1, 16, 16), Vector3(1, 16, 0),
                               Vector3(1, 0, 0));
            BuildGUIBlockPlane(blockType, 2, Vector3(15, 0, 0), Vector3(15, 16, 0), Vector3(15, 16, 16),
                               Vector3(15, 0, 16));
            BuildGUIBlockPlane(blockType, 3, Vector3(1, 0, 1), Vector3(15, 0, 1), Vector3(15, 0, 15),
                               Vector3(1, 0, 15));
            BuildGUIBlockPlane(blockType, 4, Vector3(0, 16, 15), Vector3(15, 16, 15), Vector3(15, 16, 1),
                               Vector3(1, 16, 1));
            BuildGUIBlockPlane(blockType, 5, Vector3(0, 16, 1), Vector3(16, 16, 1), Vector3(16, 0, 1),
                               Vector3(0, 0, 1));
            BuildGUIBlockPlane(blockType, 6, Vector3(0, 0, 15), Vector3(16, 0, 15), Vector3(16, 16, 15),
                               Vector3(0, 16, 15));
        }

        if (i == ChestBlock3::getID()) {
            BuildGUIBlockPlane(blockType, 1, Vector3(1, 0, 15), Vector3(1, 14, 15), Vector3(1, 14, 1),
                               Vector3(1, 0, 1));
            BuildGUIBlockPlane(blockType, 2, Vector3(15, 0, 1), Vector3(15, 14, 1), Vector3(15, 14, 15),
                               Vector3(15, 0, 15));
            BuildGUIBlockPlane(blockType, 3, Vector3(1, 0, 1), Vector3(15, 0, 1), Vector3(15, 0, 15),
                               Vector3(1, 0, 15));
            BuildGUIBlockPlane(blockType, 4, Vector3(1, 14, 15), Vector3(15, 14, 15), Vector3(15, 14, 1),
                               Vector3(1, 14, 1));
            BuildGUIBlockPlane(blockType, 5, Vector3(1, 14, 1), Vector3(15, 14, 1), Vector3(15, 0, 1),
                               Vector3(1, 0, 1));
            BuildGUIBlockPlane(blockType, 6, Vector3(1, 0, 15), Vector3(15, 0, 15), Vector3(15, 14, 15),
                               Vector3(1, 14, 15));
            ...
            BuildGUIBlockPlane(blockType, 4, Vector3(7, 11, 1), Vector3(9, 11, 1), Vector3(9, 11, 0),
                               Vector3(7, 11, 0));
    ...
    if (blockType->blockModel == 6) {
        ...
        BuildGUIBlockPlane(blockType, 2, Vector3(16, 8, 8), Vector3(16, 16, 8), Vector3(16, 16, 16),
                           Vector3(16, 8, 16));
        BuildGUIBlockPlane(blockType, 4, Vector3(0, 16, 16), Vector3(16, 16, 16), Vector3(16, 16, 8),
                           Vector3(0, 16, 8));
        BuildGUIBlockPlane(blockType, 5, Vector3(0, 16, 8), Vector3(16, 16, 8), Vector3(16, 8, 8), Vector3(0, 8, 8));
        BuildGUIBlockPlane(blockType, 6, Vector3(0, 8, 16), Vector3(16, 8, 16), Vector3(16, 16, 16),
                           Vector3(0, 16, 16));
    }
    ...
    if (blockType->blockModel == 2) {
        ...
        BuildGUIBlockPlane(blockType, 2, Vector3(10, 0, 12), Vector3(10, 16, 12), Vector3(10, 16, 16),
                           Vector3(10, 0, 16));
        BuildGUIBlockPlane(blockType, 3, Vector3(6, 0, 12), Vector3(10, 0, 12), Vector3(10, 0, 16), Vector3(6, 0, 16));
        BuildGUIBlockPlane(blockType, 4, Vector3(6, 16, 16), Vector3(10, 16, 16), Vector3(10, 16, 12),
                           Vector3(6, 16, 12));
        BuildGUIBlockPlane(blockType, 5, Vector3(6, 16, 12), Vector3(10, 16, 12), Vector3(10, 0, 12),
                           Vector3(6, 0, 12));
        BuildGUIBlockPlane(blockType, 6, Vector3(6, 0, 16), Vector3(10, 0, 16), Vector3(10, 16, 16),
                           Vector3(6, 16, 16));
    ...
    blockTypes[i].verticesLeftRight = (TexturesPSPVertex *) memalign(16, (mTriangleLeftRight.size() * 3) *
                                                                         sizeof(TexturesPSPVertex));
    ...
    sceKernelDcacheWritebackInvalidateRange(blockTypes[i].verticesLeftRight,
                                            (mTriangleLeftRight.size() * 3) * sizeof(TexturesPSPVertex));
    ...
    blockTypes[i].verticesBackFront = (TexturesPSPVertex *) memalign(16, (mTriangleBackFront.size() * 3) *
                                                                         sizeof(TexturesPSPVertex));
    ...
    //clear the cache or there will be some errors
    sceKernelDcacheWritebackInvalidateRange(blockTypes[i].verticesBackFront,
                                            (mTriangleBackFront.size() * 3) * sizeof(TexturesPSPVertex));
    //sceKernelDcacheWritebackInvalidateAll();
    ...
    blockTypes[i].verticesBottomTop = (TexturesPSPVertex *) memalign(16, (mTriangleBottomTop.size() * 3) *
                                                                         sizeof(TexturesPSPVertex));
    ...
    //clear the cache or there will be some errors
    sceKernelDcacheWritebackInvalidateRange(blockTypes[i].verticesBottomTop,
                                            (mTriangleBottomTop.size() * 3) * sizeof(TexturesPSPVertex));
    //sceKernelDcacheWritebackInvalidateAll();
...
void CraftWorld::GetCloudsVerts(BaseBlock *blockType) {
    ...
    blockTypes[141].verticesBottomTop = (TexturesPSPVertex *) memalign(16, (mTriangle.size() * 3) *
                                                                           sizeof(TexturesPSPVertex));
    ...
    //clear the cache or there will be some errors
    sceKernelDcacheWritebackInvalidateRange(blockTypes[141].verticesBottomTop,
...
void CraftWorld::GetItemVerts(Item *itemType) {
    std::vector<Vector3 *> mPosition;
    std::vector<Vector2 *> mtextures;
    std::vector<Vector3 *> mTriangle;
    ...
    itemType->vertices = (TexturesPSPVertex *) memalign(16, (mTriangle.size() * 3) * sizeof(TexturesPSPVertex));
    itemType->points = points;
    //build verts
    //vertices
    int vert = 0;
    unsigned int size = mTriangle.size();
    for (unsigned int j = 0; j < size; j++) {
        itemType->vertices[vert].u = mtextures[mTriangle[j]->x]->x;
        itemType->vertices[vert].v = mtextures[mTriangle[j]->x]->y;
        itemType->vertices[vert].x = mPosition[mTriangle[j]->x]->x;
        itemType->vertices[vert].y = mPosition[mTriangle[j]->x]->y;
        itemType->vertices[vert].z = mPosition[mTriangle[j]->x]->z;
        vert++;

        itemType->vertices[vert].u = mtextures[mTriangle[j]->y]->x;
        itemType->vertices[vert].v = mtextures[mTriangle[j]->y]->y;
        itemType->vertices[vert].x = mPosition[mTriangle[j]->y]->x;
        itemType->vertices[vert].y = mPosition[mTriangle[j]->y]->y;
        itemType->vertices[vert].z = mPosition[mTriangle[j]->y]->z;
        vert++;

        itemType->vertices[vert].u = mtextures[mTriangle[j]->z]->x;
        itemType->vertices[vert].v = mtextures[mTriangle[j]->z]->y;
        itemType->vertices[vert].x = mPosition[mTriangle[j]->z]->x;
        itemType->vertices[vert].y = mPosition[mTriangle[j]->z]->y;
        itemType->vertices[vert].z = mPosition[mTriangle[j]->z]->z;
        vert++;
    }

    //clear the cache or there will be some errors
    sceKernelDcacheWritebackInvalidateRange(itemType->vertices, (mTriangle.size() * 3) * sizeof(TexturesPSPVertex));
    //sceKernelDcacheWritebackInvalidateAll();
    ...
    itemType->dropVertices = (TexturesPSPVertex *) memalign(16, (mTriangle.size() * 3) * sizeof(TexturesPSPVertex));
    //build verts
    //vertices
    vert = 0;
    size = mTriangle.size();
    for (unsigned int j = 0; j < size; j++) {
        itemType->dropVertices[vert].u = mtextures[mTriangle[j]->x]->x;
        itemType->dropVertices[vert].v = mtextures[mTriangle[j]->x]->y;
        itemType->dropVertices[vert].x = mPosition[mTriangle[j]->x]->x;
        itemType->dropVertices[vert].y = mPosition[mTriangle[j]->x]->y;
        itemType->dropVertices[vert].z = mPosition[mTriangle[j]->x]->z;
        vert++;

        itemType->dropVertices[vert].u = mtextures[mTriangle[j]->y]->x;
        itemType->dropVertices[vert].v = mtextures[mTriangle[j]->y]->y;
        itemType->dropVertices[vert].x = mPosition[mTriangle[j]->y]->x;
        itemType->dropVertices[vert].y = mPosition[mTriangle[j]->y]->y;
        itemType->dropVertices[vert].z = mPosition[mTriangle[j]->y]->z;
        vert++;

        itemType->dropVertices[vert].u = mtextures[mTriangle[j]->z]->x;
        itemType->dropVertices[vert].v = mtextures[mTriangle[j]->z]->y;
        itemType->dropVertices[vert].x = mPosition[mTriangle[j]->z]->x;
        itemType->dropVertices[vert].y = mPosition[mTriangle[j]->z]->y;
        itemType->dropVertices[vert].z = mPosition[mTriangle[j]->z]->z;
        vert++;
    }

    //clear the cache or there will be some errors
    sceKernelDcacheWritebackInvalidateRange(itemType->dropVertices,
                                            (mTriangle.size() * 3) * sizeof(TexturesPSPVertex));
    //sceKernelDcacheWritebackInvalidateAll();
    ...
    int texture;
    itemType->terrainTexture == true ? texture = textureTerrainId : texture = textureItemsId;
    ...
    itemType->addVertices = (TexturesPSPVertex *) memalign(16, (mTriangle.size() * 3) * sizeof(TexturesPSPVertex));
    itemType->addPoints = points;
    ...
    size = mTriangle.size();
    for (unsigned int j = 0; j < size; j++) {
        itemType->addVertices[vert].u = mtextures[mTriangle[j]->x]->x;
        itemType->addVertices[vert].v = mtextures[mTriangle[j]->x]->y;
        itemType->addVertices[vert].x = mPosition[mTriangle[j]->x]->x;
        itemType->addVertices[vert].y = mPosition[mTriangle[j]->x]->y;
        itemType->addVertices[vert].z = mPosition[mTriangle[j]->x]->z;
        vert++;

        itemType->addVertices[vert].u = mtextures[mTriangle[j]->y]->x;
        itemType->addVertices[vert].v = mtextures[mTriangle[j]->y]->y;
        itemType->addVertices[vert].x = mPosition[mTriangle[j]->y]->x;
        itemType->addVertices[vert].y = mPosition[mTriangle[j]->y]->y;
        itemType->addVertices[vert].z = mPosition[mTriangle[j]->y]->z;
        vert++;

        itemType->addVertices[vert].u = mtextures[mTriangle[j]->z]->x;
        itemType->addVertices[vert].v = mtextures[mTriangle[j]->z]->y;
        itemType->addVertices[vert].x = mPosition[mTriangle[j]->z]->x;
        itemType->addVertices[vert].y = mPosition[mTriangle[j]->z]->y;
        itemType->addVertices[vert].z = mPosition[mTriangle[j]->z]->z;
        vert++;
    }

    //clear the cache or there will be some errors
    sceKernelDcacheWritebackInvalidateRange(itemType->addVertices,
                                            (mTriangle.size() * 3) * sizeof(TexturesPSPVertex));
    //sceKernelDcacheWritebackInvalidateAll();
    ...
    itemType->add2Vertices = (TexturesPSPVertex *) memalign(16, (mTriangle2.size() * 3) * sizeof(TexturesPSPVertex));
    itemType->add2Points = points2;
    ...
    for (unsigned int j = 0; j < size; j++) {
        itemType->add2Vertices[vert].u = mtextures2[mTriangle2[j]->x]->x;
        itemType->add2Vertices[vert].v = mtextures2[mTriangle2[j]->x]->y;
        itemType->add2Vertices[vert].x = mPosition2[mTriangle2[j]->x]->x;
        itemType->add2Vertices[vert].y = mPosition2[mTriangle2[j]->x]->y;
        itemType->add2Vertices[vert].z = mPosition2[mTriangle2[j]->x]->z;
        vert++;

        itemType->add2Vertices[vert].u = mtextures2[mTriangle2[j]->y]->x;
        itemType->add2Vertices[vert].v = mtextures2[mTriangle2[j]->y]->y;
        itemType->add2Vertices[vert].x = mPosition2[mTriangle2[j]->y]->x;
        itemType->add2Vertices[vert].y = mPosition2[mTriangle2[j]->y]->y;
        itemType->add2Vertices[vert].z = mPosition2[mTriangle2[j]->y]->z;
        vert++;

        itemType->add2Vertices[vert].u = mtextures2[mTriangle2[j]->z]->x;
        itemType->add2Vertices[vert].v = mtextures2[mTriangle2[j]->z]->y;
        itemType->add2Vertices[vert].x = mPosition2[mTriangle2[j]->z]->x;
        itemType->add2Vertices[vert].y = mPosition2[mTriangle2[j]->z]->y;
        itemType->add2Vertices[vert].z = mPosition2[mTriangle2[j]->z]->z;
        vert++;
    }

    //clear the cache or there will be some errors
    sceKernelDcacheWritebackInvalidateRange(itemType->add2Vertices,
...
void CraftWorld::CreateFullMeshChunk(const int StartX, const int StartY, const int StartZ) {
    ...
    OpaqueMeshChunk->bBox = BoundingBox(Vector3(StartX, StartY, StartZ),
                                        Vector3(StartX + CHUNK_SIZE, StartY + CHUNK_SIZE, StartZ + CHUNK_SIZE));
    TransparentMeshChunk->bBox = BoundingBox(Vector3(StartX, StartY, StartZ),
                                             Vector3(StartX + CHUNK_SIZE, StartY + CHUNK_SIZE, StartZ + CHUNK_SIZE));
...
void CraftWorld::GetSpecialBlock(int x, int y, int z, int &iVertex, SimpleMeshChunk *MeshChunk, block_t Block,
    ...
    if (blockType->blockModel == 5) {
        if (Block == WaterBlock::getID()) {
            ...
            if (upper_block == Lava::getID() || upper_block == WaterBlock::getID() ||
                upper_block == IceBlock::getID()) {

                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                                     Vector3(0, 16, 16), Vector3(0, 16, 0), Vector3(0, 0, 0));
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                                     Vector3(16, 16, 0), Vector3(16, 16, 16), Vector3(16, 0, 16));

                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                     Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(0, 0, 16));
                ...
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 16, 0),
                                     Vector3(16, 16, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                                     Vector3(16, 0, 16), Vector3(16, 16, 16), Vector3(0, 16, 16));
            } else {
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                                     Vector3(0, 14, 16), Vector3(0, 14, 0), Vector3(0, 0, 0));
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                                     Vector3(16, 14, 0), Vector3(16, 14, 16), Vector3(16, 0, 16));

                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                     Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(0, 0, 16));
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 14, 16),
                                     Vector3(16, 14, 16), Vector3(16, 14, 0), Vector3(0, 14, 0), 1, 0);

                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 14, 0),
                                     Vector3(16, 14, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                                     Vector3(16, 0, 16), Vector3(16, 14, 16), Vector3(0, 14, 16));
            }
            return;
        }
        ...
        if (Block == 9 || Block == 37 || Block == 38) {
            ...
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                 Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(0, 0, 16));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 16, 16),
                                 Vector3(16, 16, 16), Vector3(16, 16, 0), Vector3(0, 16, 0));
        ...
        if (Block == Snow2::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                                 Vector3(0, 2, 16), Vector3(0, 2, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                                 Vector3(16, 2, 0), Vector3(16, 2, 16), Vector3(16, 0, 16));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                 Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(0, 0, 16));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 2, 16),
                                 Vector3(16, 2, 16), Vector3(16, 2, 0), Vector3(0, 2, 0), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 2, 0),
                                 Vector3(16, 2, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                                 Vector3(16, 0, 16), Vector3(16, 2, 16), Vector3(0, 2, 16));
        ...
        /// CACTI
        if (Block == 25) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(1, 0, 16),
                                 Vector3(1, 16, 16), Vector3(1, 16, 0), Vector3(1, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(15, 0, 0),
                                 Vector3(15, 16, 0), Vector3(15, 16, 16), Vector3(15, 0, 16));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(1, 0, 1),
                                 Vector3(15, 0, 1), Vector3(15, 0, 15), Vector3(1, 0, 15));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(1, 16, 15),
                                 Vector3(15, 16, 15), Vector3(15, 16, 1), Vector3(1, 16, 1));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 16, 1),
                                 Vector3(16, 16, 1), Vector3(16, 0, 1), Vector3(0, 0, 1));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 15),
                                 Vector3(16, 0, 15), Vector3(16, 16, 15), Vector3(0, 16, 15));
            return;
        }
        ...
        if (Block >= BlackWoolCarpet::getID() && Block <= WhiteWoolCarpet::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                                 Vector3(0, 1, 16), Vector3(0, 1, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                                 Vector3(16, 1, 0), Vector3(16, 1, 16), Vector3(16, 0, 16));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                 Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(0, 0, 16));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 1, 16),
                                 Vector3(16, 1, 16), Vector3(16, 1, 0), Vector3(0, 1, 0), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 1, 0),
                                 Vector3(16, 1, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                                 Vector3(16, 0, 16), Vector3(16, 1, 16), Vector3(0, 1, 16));
        ...
        if (Block == ItemFrame1::getID()) {
            ...
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(1, 2, 2),
                                 Vector3(1, 14, 2), Vector3(1, 14, 14), Vector3(1, 2, 14), true, false, 3);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(0.5f, 2, 2),
                                 Vector3(0.5f, 14, 2), Vector3(0.5f, 14, 14), Vector3(0.5f, 2, 14), true, false, 2);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 2, 2),
                                 Vector3(1, 2, 2), Vector3(1, 2, 14), Vector3(0, 2, 14), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 13, 3),
                                 Vector3(1, 13, 3), Vector3(1, 13, 13), Vector3(0, 13, 13), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 14, 14),
                                 Vector3(1, 14, 14), Vector3(1, 14, 2), Vector3(0, 14, 2), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 3, 13),
                                 Vector3(1, 3, 13), Vector3(1, 3, 3), Vector3(0, 3, 3), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 14, 2),
                                 Vector3(1, 14, 2), Vector3(1, 2, 2), Vector3(0, 2, 2), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 13, 13),
                                 Vector3(1, 13, 13), Vector3(1, 3, 13), Vector3(0, 3, 13), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 2, 14),
                                 Vector3(1, 2, 14), Vector3(1, 14, 14), Vector3(0, 14, 14), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 3, 3),
                                 Vector3(1, 3, 3), Vector3(1, 13, 3), Vector3(0, 13, 3), true, false, 0);

            return;
        }

        if (Block == ItemFrame2::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(15, 2, 14),
                                 Vector3(15, 14, 14), Vector3(15, 14, 2), Vector3(15, 2, 2), true, false, 3);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(15.5f, 2, 14),
                                 Vector3(15.5f, 14, 14), Vector3(15.5f, 14, 2), Vector3(15.5f, 2, 2), true, false, 2);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(15, 2, 2),
                                 Vector3(16, 2, 2), Vector3(15, 2, 14), Vector3(16, 2, 14), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(15, 13, 3),
                                 Vector3(16, 13, 3), Vector3(16, 13, 13), Vector3(15, 13, 13), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(15, 14, 14),
                                 Vector3(16, 14, 14), Vector3(16, 14, 2), Vector3(15, 14, 2), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(15, 3, 13),
                                 Vector3(16, 3, 13), Vector3(16, 3, 3), Vector3(15, 3, 3), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(15, 14, 2),
                                 Vector3(16, 14, 2), Vector3(16, 2, 2), Vector3(15, 2, 2), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(15, 13, 13),
                                 Vector3(16, 13, 13), Vector3(16, 3, 13), Vector3(15, 3, 13), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(15, 2, 14),
                                 Vector3(16, 2, 14), Vector3(16, 14, 14), Vector3(15, 14, 14), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(15, 3, 3),
                                 Vector3(16, 3, 3), Vector3(16, 13, 3), Vector3(15, 13, 3), true, false, 0);

            return;
        }

        if (Block == ItemFrame3::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(2, 2, 1),
                                 Vector3(2, 14, 1), Vector3(2, 14, 0), Vector3(2, 2, 0), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(13, 3, 1),
                                 Vector3(13, 13, 1), Vector3(13, 13, 0), Vector3(13, 3, 0), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(14, 2, 0),
                                 Vector3(14, 14, 0), Vector3(14, 14, 1), Vector3(14, 2, 1), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(3, 3, 0),
                                 Vector3(3, 13, 0), Vector3(3, 13, 1), Vector3(3, 3, 1), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(2, 2, 0),
                                 Vector3(14, 2, 0), Vector3(14, 2, 1), Vector3(2, 2, 1), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(3, 13, 0),
                                 Vector3(13, 13, 0), Vector3(13, 13, 1), Vector3(3, 13, 1), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(2, 14, 1),
                                 Vector3(14, 14, 1), Vector3(14, 14, 0), Vector3(2, 14, 0), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(3, 3, 1),
                                 Vector3(13, 3, 1), Vector3(13, 3, 0), Vector3(3, 3, 0), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(2, 2, 1),
                                 Vector3(14, 2, 1), Vector3(14, 14, 1), Vector3(2, 14, 1), true, false, 3);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(2, 2, 0.5f),
                                 Vector3(14, 2, 0.5f), Vector3(14, 14, 0.5f), Vector3(2, 14, 0.5f), true, false, 2);

            return;
        }

        if (Block == ItemFrame4::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(2, 2, 16),
                                 Vector3(2, 14, 16), Vector3(2, 14, 15), Vector3(2, 2, 15), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(13, 3, 16),
                                 Vector3(13, 13, 16), Vector3(13, 13, 15), Vector3(13, 3, 15), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(14, 2, 15),
                                 Vector3(14, 14, 15), Vector3(14, 14, 16), Vector3(14, 2, 16), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(3, 3, 15),
                                 Vector3(3, 13, 15), Vector3(3, 13, 16), Vector3(3, 3, 16), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(2, 2, 15),
                                 Vector3(14, 2, 15), Vector3(14, 2, 16), Vector3(2, 2, 16), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(3, 13, 15),
                                 Vector3(13, 13, 15), Vector3(13, 13, 16), Vector3(3, 13, 16), true, false, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(2, 14, 16),
                                 Vector3(14, 14, 16), Vector3(14, 14, 15), Vector3(2, 14, 15), true, false, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(3, 3, 16),
                                 Vector3(13, 3, 16), Vector3(13, 3, 15), Vector3(3, 3, 15), true, false, 0);


            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(2, 14, 15),
                                 Vector3(14, 14, 15), Vector3(14, 2, 15), Vector3(2, 2, 15), true, false, 3);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(2, 14, 15.5f),
                                 Vector3(14, 14, 15.5f), Vector3(14, 2, 15.5f), Vector3(2, 2, 15.5f), true, false, 2);
        ...
        if (Block >= TrapdoorClosed1::getID() && Block <= TrapdoorClosed4::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                                 Vector3(0, 3, 16), Vector3(0, 3, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                                 Vector3(16, 3, 0), Vector3(16, 3, 16), Vector3(16, 0, 16));

            if (BlockTransparentOrSpecial(x, y - 1, z)) {
                BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3,
                                            Vector3(0, 0, 0), Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(0, 0, 16));
            }
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 3, 16),
                                 Vector3(16, 3, 16), Vector3(16, 3, 0), Vector3(0, 3, 0));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 3, 0),
                                 Vector3(16, 3, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                                 Vector3(16, 0, 16), Vector3(16, 3, 16), Vector3(0, 3, 16));
            return;
        }
        ...
        if (Block == TrapdoorOpen1::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                                 Vector3(0, 16, 16), Vector3(0, 16, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(3, 0, 0),
                                        Vector3(3, 16, 0), Vector3(3, 16, 16), Vector3(3, 0, 16));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                 Vector3(3, 0, 0), Vector3(3, 0, 16), Vector3(0, 0, 16));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 16, 16),
                                 Vector3(3, 16, 16), Vector3(3, 16, 0), Vector3(0, 16, 0));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 16, 0),
                                 Vector3(3, 16, 0), Vector3(3, 0, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                                 Vector3(3, 0, 16), Vector3(3, 16, 16), Vector3(0, 16, 16));
            return;
        }

        if (Block == TrapdoorOpen2::getID()) {
            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1,
                                        Vector3(13, 0, 16), Vector3(13, 16, 16), Vector3(13, 16, 0), Vector3(13, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                                 Vector3(16, 16, 0), Vector3(16, 16, 16), Vector3(16, 0, 16));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(13, 0, 0),
                                 Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(13, 0, 16));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(13, 16, 16),
                                 Vector3(16, 16, 16), Vector3(16, 16, 0), Vector3(13, 16, 0));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(13, 16, 0),
                                 Vector3(16, 16, 0), Vector3(16, 0, 0), Vector3(13, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(13, 0, 16),
                                 Vector3(16, 0, 16), Vector3(16, 16, 16), Vector3(13, 16, 16));
            return;
        }

        if (Block == TrapdoorOpen3::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 3),
                                 Vector3(0, 16, 3), Vector3(0, 16, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                                 Vector3(16, 16, 0), Vector3(16, 16, 3), Vector3(16, 0, 3));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                 Vector3(16, 0, 0), Vector3(16, 0, 3), Vector3(0, 0, 3));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 16, 3),
                                 Vector3(16, 16, 3), Vector3(16, 16, 0), Vector3(0, 16, 0));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 16, 0),
                                 Vector3(16, 16, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 3),
                                        Vector3(16, 0, 3), Vector3(16, 16, 3), Vector3(0, 16, 3));
            return;
        }

        if (Block == TrapdoorOpen4::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                                 Vector3(0, 16, 16), Vector3(0, 16, 13), Vector3(0, 0, 13));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 13),
                                 Vector3(16, 16, 13), Vector3(16, 16, 16), Vector3(16, 0, 16));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 13),
                                 Vector3(16, 0, 13), Vector3(16, 0, 16), Vector3(0, 0, 16));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 16, 16),
                                 Vector3(16, 16, 16), Vector3(16, 16, 13), Vector3(0, 16, 13));

            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5,
                                        Vector3(0, 16, 13), Vector3(16, 16, 13), Vector3(16, 0, 13), Vector3(0, 0, 13));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                                 Vector3(16, 0, 16), Vector3(16, 16, 16), Vector3(0, 16, 16));
            return;
        }
        ...
        if (Block == Ladder1::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(0.5f, 0, 0),
                                 Vector3(0.5f, 16, 0), Vector3(0.5f, 16, 16), Vector3(0.5f, 0, 16), 1, 1);
            return;
        }

        if (Block == Ladder2::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(15.5f, 0, 16),
                                 Vector3(15.5f, 16, 16), Vector3(15.5f, 16, 0), Vector3(15.5f, 0, 0), 1, 1);
            return;
        }

        if (Block == Ladder3::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 0.5f),
                                 Vector3(16, 0, 0.5f), Vector3(16, 16, 0.5f), Vector3(0, 16, 0.5f), 1, 1);
            return;
        }

        if (Block == Ladder4::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 16, 15.5f),
                                 Vector3(16, 16, 15.5f), Vector3(16, 0, 15.5f), Vector3(0, 0, 15.5f), 1, 1);
            return;
        }
        ...
        if (Block == 112) {
            ...
            if (upper_block == WaterBlock::getID() || upper_block == Lava::getID()) {
                ...
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                     Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(0, 0, 16));
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 14, 16),
                                     Vector3(16, 14, 16), Vector3(16, 14, 0), Vector3(0, 14, 0));
        ...
        if (Block == MelonPlant4::getID()) {
            ...
            if (GetBlock(x + 1, y, z) == MelonBlock::getID()) {
                ...
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 16, 8),
                                     Vector3(16, 16, 8), Vector3(16, 0, 8), Vector3(0, 0, 8), 1, 1);
                blockType = &blockTypes[MelonPlant4Reverse::getID()];
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, MelonPlant4Reverse::getID(), transparent,
                                     6, Vector3(0, 0, 8), Vector3(16, 0, 8), Vector3(16, 16, 8), Vector3(0, 16, 8), 1,
                                     1);
                haveMelon = true;
            }
            if (GetBlock(x - 1, y, z) == MelonBlock::getID() && haveMelon == false) {
                blockType = &blockTypes[MelonPlant4Reverse::getID()];
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, MelonPlant4Reverse::getID(), transparent,
                                     5, Vector3(0, 16, 8), Vector3(16, 16, 8), Vector3(16, 0, 8), Vector3(0, 0, 8), 1,
                                     1);
                blockType = &blockTypes[Block];
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 8),
                                     Vector3(16, 0, 8), Vector3(16, 16, 8), Vector3(0, 16, 8), 1, 1);
                haveMelon = true;
            }
            if (GetBlock(x, y, z + 1) == MelonBlock::getID() && haveMelon == false) {
                blockType = &blockTypes[MelonPlant4Reverse::getID()];
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, MelonPlant4Reverse::getID(), transparent,
                                     1, Vector3(8, 0, 16), Vector3(8, 16, 16), Vector3(8, 16, 0), Vector3(8, 0, 0), 1,
                                     1);
                blockType = &blockTypes[Block];
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(8, 0, 0),
                                     Vector3(8, 16, 0), Vector3(8, 16, 16), Vector3(8, 0, 16), 1, 1);
                haveMelon = true;
            }
            if (GetBlock(x, y, z - 1) == MelonBlock::getID() && haveMelon == false) {
                blockType = &blockTypes[Block];
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(8, 0, 16),
                                     Vector3(8, 16, 16), Vector3(8, 16, 0), Vector3(8, 0, 0), 1, 1);
                blockType = &blockTypes[MelonPlant4Reverse::getID()];
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, MelonPlant4Reverse::getID(), transparent,
                                     2, Vector3(8, 0, 0), Vector3(8, 16, 0), Vector3(8, 16, 16), Vector3(8, 0, 16), 1,
                                     1);
                haveMelon = true;
            }
        ...
        if (Block == PumpkinPlant4::getID()) {
            ...
            if (GetBlock(x + 1, y, z) == Pumpkin1::getID()) {
                ...
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, PumpkinPlant4Reverse::getID(), transparent,
                                     6, Vector3(0, 0, 8), Vector3(16, 0, 8), Vector3(16, 16, 8), Vector3(0, 16, 8), 1,
                                     1);
                havePumpkin = true;
            }
        ...
        if (
                Block == 49 ||
                (Block == 56 && GetBlock(x, y, z - 1) != 52 && GetBlock(x, y, z - 1) != 56 &&
                 (GetBlock(x, y, z + 1) == 56 || GetBlock(x, y, z + 1) == 52)) ||
                (Block == 56 && GetBlock(x, y, z - 1) != 52 && GetBlock(x, y, z - 1) != 56 &&
                 GetBlock(x, y, z + 1) != 52 && GetBlock(x, y, z + 1) != 56) ||
                (Block == 55 && GetBlock(x, y, z - 1) != 51 && GetBlock(x, y, z - 1) != 55 &&
                 (GetBlock(x, y, z + 1) == 55 || GetBlock(x, y, z + 1) == 51))
                ) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 3),
                                 Vector3(0, 32, 3), Vector3(0, 32, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                                 Vector3(16, 32, 0), Vector3(16, 32, 3), Vector3(16, 0, 3));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                 Vector3(16, 0, 0), Vector3(16, 0, 3), Vector3(0, 0, 3));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 32, 3),
                                 Vector3(16, 32, 3), Vector3(16, 32, 0), Vector3(0, 32, 0));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 32, 0),
                                 Vector3(16, 32, 0), Vector3(16, 0, 0), Vector3(0, 0, 0), 0, 1);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 3),
                                 Vector3(16, 0, 3), Vector3(16, 32, 3), Vector3(0, 32, 3), 1, 1);
            return;
        }
        ...
        if (
                ...
                (Block == 55 && GetBlock(x, y, z + 1) != 51 && GetBlock(x, y, z + 1) != 55 &&
                 (GetBlock(x, y, z - 1) == 55 || GetBlock(x, y, z - 1) == 51)) ||
                (Block == 55 && GetBlock(x, y, z + 1) != 51 && GetBlock(x, y, z + 1) != 55 &&
                 GetBlock(x, y, z - 1) != 51 && GetBlock(x, y, z - 1) != 55)
                ) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                                 Vector3(0, 32, 16), Vector3(0, 32, 13), Vector3(0, 0, 13));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 13),
                                 Vector3(16, 32, 13), Vector3(16, 32, 16), Vector3(16, 0, 16));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 13),
                                 Vector3(16, 0, 13), Vector3(16, 0, 16), Vector3(0, 0, 16));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 32, 16),
                                 Vector3(16, 32, 16), Vector3(16, 32, 13), Vector3(0, 32, 13));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 32, 13),
                                 Vector3(16, 32, 13), Vector3(16, 0, 13), Vector3(0, 0, 13), 1, 1);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                                 Vector3(16, 0, 16), Vector3(16, 32, 16), Vector3(0, 32, 16), 0, 1);
            return;
        }
        ...
        if (
                Block == 51 ||
                (Block == 53 && GetBlock(x - 1, y, z) != 49 && GetBlock(x - 1, y, z) != 53 &&
                 (GetBlock(x + 1, y, z) == 53 || GetBlock(x + 1, y, z) == 49)) ||
                (Block == 53 && GetBlock(x - 1, y, z) != 49 && GetBlock(x - 1, y, z) != 53 &&
                 GetBlock(x + 1, y, z) != 49 && GetBlock(x + 1, y, z) != 53) ||
                (Block == 54 && GetBlock(x - 1, y, z) != 50 && GetBlock(x - 1, y, z) != 54 &&
                 (GetBlock(x + 1, y, z) == 54 || GetBlock(x + 1, y, z) == 50))
                ) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                                 Vector3(0, 32, 16), Vector3(0, 32, 0), Vector3(0, 0, 0), 0, 1);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(3, 0, 0),
                                 Vector3(3, 32, 0), Vector3(3, 32, 16), Vector3(3, 0, 16), 1, 1);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                 Vector3(3, 0, 0), Vector3(3, 0, 16), Vector3(0, 0, 16));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 32, 16),
                                 Vector3(3, 32, 16), Vector3(3, 32, 0), Vector3(0, 32, 0));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 32, 0),
                                 Vector3(3, 32, 0), Vector3(3, 0, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                                 Vector3(3, 0, 16), Vector3(3, 32, 16), Vector3(0, 32, 16));
            return;
        }
        ...
        if (
                ...
                (Block == 54 && GetBlock(x + 1, y, z) != 50 && GetBlock(x + 1, y, z) != 54 &&
                 (GetBlock(x - 1, y, z) == 54 || GetBlock(x - 1, y, z) == 50)) ||
                (Block == 54 && GetBlock(x + 1, y, z) != 50 && GetBlock(x + 1, y, z) != 54 &&
                 GetBlock(x - 1, y, z) != 50 && GetBlock(x - 1, y, z) != 54)
                ) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(13, 0, 16),
                                 Vector3(13, 32, 16), Vector3(13, 32, 0), Vector3(13, 0, 0), 1, 1);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                                 Vector3(16, 32, 0), Vector3(16, 32, 16), Vector3(16, 0, 16), 0, 1);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(13, 0, 0),
                                 Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(13, 0, 16));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(13, 32, 16),
                                 Vector3(16, 32, 16), Vector3(16, 32, 0), Vector3(13, 32, 0));

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(13, 32, 0),
                                 Vector3(16, 32, 0), Vector3(16, 0, 0), Vector3(13, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(13, 0, 16),
                                 Vector3(16, 0, 16), Vector3(16, 32, 16), Vector3(13, 32, 16));
            return;
        }
        ...
        if (Block == 94) {
            // chest
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(1, 0, 15),
                                 Vector3(1, 14, 15), Vector3(1, 14, 1), Vector3(1, 0, 1), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(15, 0, 1),
                                 Vector3(15, 14, 1), Vector3(15, 14, 15), Vector3(15, 0, 15), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(1, 0, 1),
                                 Vector3(15, 0, 1), Vector3(15, 0, 15), Vector3(1, 0, 15), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(1, 14, 15),
                                 Vector3(15, 14, 15), Vector3(15, 14, 1), Vector3(1, 14, 1), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(1, 14, 1),
                                 Vector3(15, 14, 1), Vector3(15, 0, 1), Vector3(1, 0, 1), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(1, 0, 15),
                                 Vector3(15, 0, 15), Vector3(15, 14, 15), Vector3(1, 14, 15), 1, 0);

            // chest lock
            blockType = &blockTypes[98];
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 1, Vector3(0, 7, 9),
                                 Vector3(0, 11, 9), Vector3(0, 11, 7), Vector3(0, 7, 7), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 3, Vector3(0, 7, 7),
                                 Vector3(1, 7, 7), Vector3(1, 7, 9), Vector3(0, 7, 9), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 4, Vector3(0, 11, 9),
                                 Vector3(1, 11, 9), Vector3(1, 11, 7), Vector3(0, 11, 7), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 5, Vector3(0, 11, 7),
                                 Vector3(1, 11, 7), Vector3(1, 7, 7), Vector3(0, 7, 7), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 6, Vector3(0, 7, 9),
                                 Vector3(1, 7, 9), Vector3(1, 11, 9), Vector3(0, 11, 9), 1, 0);
            return;
        }
        ...
        if (Block == 95) {
            ...
            // chest lock
            blockType = &blockTypes[98];
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 2, Vector3(16, 7, 7),
                                 Vector3(16, 11, 7), Vector3(16, 11, 9), Vector3(16, 7, 9), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 3, Vector3(15, 7, 7),
                                 Vector3(16, 7, 7), Vector3(16, 7, 9), Vector3(15, 7, 9), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 4, Vector3(15, 11, 9),
                                 Vector3(16, 11, 9), Vector3(16, 11, 7), Vector3(15, 11, 7), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 5, Vector3(15, 11, 7),
                                 Vector3(16, 11, 7), Vector3(16, 7, 7), Vector3(15, 7, 7), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 6, Vector3(15, 7, 9),
                                 Vector3(16, 7, 9), Vector3(16, 11, 9), Vector3(15, 11, 9), 1, 0);
            return;
        }
        ...
        if (Block == 96) {
            ...
            // chest lock
            blockType = &blockTypes[98];
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 1, Vector3(7, 7, 1),
                                 Vector3(7, 11, 1), Vector3(7, 11, 0), Vector3(7, 7, 0), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 2, Vector3(9, 7, 0),
                                 Vector3(9, 11, 0), Vector3(9, 11, 1), Vector3(9, 7, 1), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 3, Vector3(7, 7, 0),
                                 Vector3(9, 7, 0), Vector3(9, 7, 1), Vector3(7, 7, 1), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 4, Vector3(7, 11, 1),
                                 Vector3(9, 11, 1), Vector3(9, 11, 0), Vector3(7, 11, 0), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 5, Vector3(7, 11, 0),
                                 Vector3(9, 11, 0), Vector3(9, 7, 0), Vector3(7, 7, 0), 1, 0);
            return;
        }
        ...
        if (Block == 97) {
            ...
            // chest lock
            blockType = &blockTypes[98];
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 1, Vector3(7, 7, 16),
                                 Vector3(7, 11, 16), Vector3(7, 11, 15), Vector3(7, 7, 15), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 2, Vector3(9, 7, 15),
                                 Vector3(9, 11, 15), Vector3(9, 11, 16), Vector3(9, 7, 16), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 3, Vector3(7, 7, 15),
                                 Vector3(9, 7, 15), Vector3(9, 7, 16), Vector3(7, 7, 16), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 4, Vector3(7, 11, 16),
                                 Vector3(9, 11, 16), Vector3(9, 11, 15), Vector3(7, 11, 15), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, 66, transparent, 6, Vector3(7, 7, 16),
                                 Vector3(9, 7, 16), Vector3(9, 11, 16), Vector3(7, 11, 16), 1, 0);
            return;
        }
        ...
        if (Block == Torch::getID()) {
            ...
            if (BlockSolid(x, y - 1, z)) /////
            {
                ...
                MeshChunk->vert(x + 0.4375f + pixel * 2, y + pixel * 11, z + 0.4375f + pixel * 2 + pixel, left, up, 1,
                                1, 1);
                ...
                MeshChunk->vert(x + 0.4375f + pixel * 2 + pixel, y + pixel * 11, z + 0.4375f + pixel * 2, right, up, 1,
                                1, 1);
                ...
                //y+1
                {
                    ...
                    MeshChunk->vert(x + 0.4375f + pixel * 2, y + pixel * 10, z + 0.4375f + pixel * 2, right, up, 1, 1,
                                    1);
                ...
                if (BlockSolid(x - 1, y, z) && canPlaseTorch) {
                    ...
                    MeshChunk->vert(x + pixel * 4 - pixel / 2.0f, y + pixel * 13, z + pixel * 7 + pixel * 2, left, up,
                                    1, 1, 1);
                    MeshChunk->vert(x + pixel * 4 + pixel * 2 - pixel / 2.0f, y + pixel * 13, z + pixel * 7 + pixel * 2,
                                    right, up, 1, 1, 1);
                    MeshChunk->vert(x + pixel * 4 + pixel * 2 - pixel / 2.0f, y + pixel * 13, z + pixel * 7, right,
                                    down, 1, 1, 1);
                ...
                if (BlockSolid(x + 1, y, z) && canPlaseTorch) {
                    //x-1
                    MeshChunk->vert(x + pixel * 15, y + pixel * 3, z + 0.4375f + pixel * 2 + pixel, right, down, 1, 1,
                                    1);
                    MeshChunk->vert(x + pixel * 10, y + pixel * 14, z + 0.4375f + pixel * 2 + pixel, right, up, 1, 1,
                                    1);
                    ...
                    MeshChunk->vert(x + pixel * 17, y + pixel * 3, z + 0.4375f + pixel * 2 + pixel, left, down, 1, 1,
                                    1);
...

                    {
                        ...
                        MeshChunk->vert(x + pixel * 10 + pixel / 2.0f, y + pixel * 13, z + pixel * 7 + pixel * 2, left,
                                        up, 1, 1, 1);
                        MeshChunk->vert(x + pixel * 12 + pixel / 2.0f, y + pixel * 13, z + pixel * 7 + pixel * 2, right,
                                        up, 1, 1, 1);
                        MeshChunk->vert(x + pixel * 12 + pixel / 2.0f, y + pixel * 13, z + pixel * 7, right, down, 1, 1,
                                        1);
                        MeshChunk->vert(x + pixel * 10 + pixel / 2.0f, y + pixel * 13, z + pixel * 7, left, down, 1, 1,
                                        1);
                ...
                if (BlockSolid(x, y, z - 1) && canPlaseTorch) {
                    ...
                    MeshChunk->vert(x + pixel * 7 + pixel * 2 + pixel, y + pixel * 14, z + pixel * 4, left, up, 1, 1,
                                    1);
                    ...
                    MeshChunk->vert(x + pixel * 7 + pixel * 2 + pixel, y + pixel * 14, z + pixel * 6, right, up, 1, 1,
                                    1);
                    ...
                    //y+1
                    {
                        ...
                        MeshChunk->vert(x + pixel * 7 + pixel * 2, y + pixel * 13, z + pixel * 6 - pixel / 2.0f, right,
                                        up, 1, 1, 1);
                        MeshChunk->vert(x + pixel * 7 + pixel * 2, y + pixel * 13, z + pixel * 4 - pixel / 2.0f, right,
                                        down, 1, 1, 1);
                        MeshChunk->vert(x + pixel * 7, y + pixel * 13, z + pixel * 4 - pixel / 2.0f, left, down, 1, 1,
                                        1);
                ...
                if (BlockSolid(x, y, z + 1) && canPlaseTorch) {
                    ...
                    MeshChunk->vert(x + pixel * 7 + pixel * 2 + pixel, y + pixel * 14, z + pixel * 10, left, up, 1, 1,
                                    1);
                    MeshChunk->vert(x + pixel * 7 + pixel * 2 + pixel, y + pixel * 3, z + pixel * 15, left, down, 1, 1,
                                    1);
                    ...
                    MeshChunk->vert(x + pixel * 7 + pixel * 2 + pixel, y + pixel * 3, z + pixel * 17, right, down, 1, 1,
                                    1);
                    MeshChunk->vert(x + pixel * 7 + pixel * 2 + pixel, y + pixel * 14, z + pixel * 12, right, up, 1, 1,
                                    1);
                    ...
                    //y+1
                    {
                        ...
                        MeshChunk->vert(x + pixel * 7, y + pixel * 13, z + pixel * 12 + pixel / 2.0f, left, up, 1, 1,
                                        1);
                        MeshChunk->vert(x + pixel * 7 + pixel * 2, y + pixel * 13, z + pixel * 12 + pixel / 2.0f, right,
                                        up, 1, 1, 1);
                        MeshChunk->vert(x + pixel * 7 + pixel * 2, y + pixel * 13, z + pixel * 10 + pixel / 2.0f, right,
                                        down, 1, 1, 1);
                        MeshChunk->vert(x + pixel * 7, y + pixel * 13, z + pixel * 10 + pixel / 2.0f, left, down, 1, 1,
                                        1);
    ...
    if (blockType->blockModel == 1) {
        ...
        if (Block == BedBlock1::getID()) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                                 Vector3(0, 9, 16), Vector3(0, 9, 0), Vector3(0, 0, 0));
            ...
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 3, 0),
                                 Vector3(16, 3, 0), Vector3(16, 3, 16), Vector3(0, 3, 16));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 9, 16),
                                 Vector3(16, 9, 16), Vector3(16, 9, 0), Vector3(0, 9, 0), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 9, 0),
                                 Vector3(16, 9, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                                 Vector3(16, 0, 16), Vector3(16, 9, 16), Vector3(0, 9, 16));
            return;
        }
        if (Block == BedBlock5::getID()) {
            ...
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                                 Vector3(16, 9, 0), Vector3(16, 9, 16), Vector3(16, 0, 16));
        ...
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                             Vector3(0, 8, 16), Vector3(0, 8, 0), Vector3(0, 0, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                             Vector3(16, 8, 0), Vector3(16, 8, 16), Vector3(16, 0, 16));

        if (BlockTransparentOrSpecial(x, y - 1, z)) {
            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 0),
                                        Vector3(16, 0, 0), Vector3(16, 0, 16), Vector3(0, 0, 16));
        }
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 8, 16),
                             Vector3(16, 8, 16), Vector3(16, 8, 0), Vector3(0, 8, 0), 1, 0);

        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 8, 0),
                             Vector3(16, 8, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                             Vector3(16, 0, 16), Vector3(16, 8, 16), Vector3(0, 8, 16));
    ...
    /// FENCE
    if (blockType->blockModel == 2) {
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(6, 0, 10),
                             Vector3(6, 16, 10), Vector3(6, 16, 6), Vector3(6, 0, 6), 1, 0);
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(10, 0, 6),
                             Vector3(10, 16, 6), Vector3(10, 16, 10), Vector3(10, 0, 10), 1, 0);

        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(6, 0, 6),
                             Vector3(10, 0, 6), Vector3(10, 0, 10), Vector3(6, 0, 10));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(6, 16, 10),
                             Vector3(10, 16, 10), Vector3(10, 16, 6), Vector3(6, 16, 6));

        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(6, 16, 6),
                             Vector3(10, 16, 6), Vector3(10, 0, 6), Vector3(6, 0, 6), 1, 0);
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(6, 0, 10),
                             Vector3(10, 0, 10), Vector3(10, 16, 10), Vector3(6, 16, 10), 1, 0);

        if (BlockSolid(x, y, z - 1) == true) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(7, 12, 8),
                                 Vector3(7, 15, 8), Vector3(7, 15, 0), Vector3(7, 12, 0), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(7, 6, 8),
                                 Vector3(7, 9, 8), Vector3(7, 9, 0), Vector3(7, 6, 0), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(9, 12, 0),
                                 Vector3(9, 15, 0), Vector3(9, 15, 8), Vector3(9, 12, 8), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(9, 6, 0),
                                 Vector3(9, 9, 0), Vector3(9, 9, 8), Vector3(9, 6, 8), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(7, 12, 0),
                                 Vector3(9, 12, 0), Vector3(9, 12, 8), Vector3(7, 12, 8), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(7, 6, 0),
                                 Vector3(9, 6, 0), Vector3(9, 6, 8), Vector3(7, 6, 8), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 15, 8),
                                 Vector3(9, 15, 8), Vector3(9, 15, 0), Vector3(7, 15, 0), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 9, 8),
                                 Vector3(9, 9, 8), Vector3(9, 9, 0), Vector3(7, 9, 0), 1, 0);
        }

        if (BlockSolid(x, y, z + 1) == true) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(7, 12, 16),
                                 Vector3(7, 15, 16), Vector3(7, 15, 8), Vector3(7, 12, 8), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(7, 6, 16),
                                 Vector3(7, 9, 16), Vector3(7, 9, 8), Vector3(7, 6, 8), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(9, 12, 8),
                                 Vector3(9, 15, 8), Vector3(9, 15, 16), Vector3(9, 12, 16), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(9, 6, 8),
                                 Vector3(9, 9, 8), Vector3(9, 9, 16), Vector3(9, 6, 16), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(7, 12, 8),
                                 Vector3(9, 12, 8), Vector3(9, 12, 16), Vector3(7, 12, 16), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(7, 6, 8),
                                 Vector3(9, 6, 8), Vector3(9, 6, 16), Vector3(7, 6, 16), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 15, 16),
                                 Vector3(9, 15, 16), Vector3(9, 15, 8), Vector3(7, 15, 8), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 9, 16),
                                 Vector3(9, 9, 16), Vector3(9, 9, 8), Vector3(7, 9, 8), 1, 0);
        }

        if (BlockSolid(x - 1, y, z) == true) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 15, 7),
                                 Vector3(8, 15, 7), Vector3(8, 12, 7), Vector3(0, 12, 7), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 9, 7),
                                 Vector3(8, 9, 7), Vector3(8, 6, 7), Vector3(0, 6, 7), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 12, 9),
                                 Vector3(8, 12, 9), Vector3(8, 15, 9), Vector3(0, 15, 9), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 6, 9),
                                 Vector3(8, 6, 9), Vector3(8, 9, 9), Vector3(0, 9, 9), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 12, 7),
                                 Vector3(8, 12, 7), Vector3(8, 12, 9), Vector3(0, 12, 9), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 6, 7),
                                 Vector3(8, 6, 7), Vector3(8, 6, 9), Vector3(0, 6, 9), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 15, 9),
                                 Vector3(8, 15, 9), Vector3(8, 15, 7), Vector3(0, 15, 7), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 9, 9),
                                 Vector3(8, 9, 9), Vector3(8, 9, 7), Vector3(0, 9, 7), 1, 0);
        }

        if (BlockSolid(x + 1, y, z) == true) {
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(8, 15, 7),
                                 Vector3(16, 15, 7), Vector3(16, 12, 7), Vector3(8, 12, 7), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(8, 9, 7),
                                 Vector3(16, 9, 7), Vector3(16, 6, 7), Vector3(8, 6, 7), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(8, 12, 9),
                                 Vector3(16, 12, 9), Vector3(16, 15, 9), Vector3(8, 15, 9), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(8, 6, 9),
                                 Vector3(16, 6, 9), Vector3(16, 9, 9), Vector3(8, 9, 9), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(8, 12, 7),
                                 Vector3(16, 12, 7), Vector3(16, 12, 9), Vector3(8, 12, 9), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(8, 6, 7),
                                 Vector3(16, 6, 7), Vector3(16, 6, 9), Vector3(8, 6, 9), 1, 0);

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(8, 15, 9),
                                 Vector3(16, 15, 9), Vector3(16, 15, 7), Vector3(8, 15, 7), 1, 0);
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(8, 9, 9),
                                 Vector3(16, 9, 9), Vector3(16, 9, 7), Vector3(8, 9, 7), 1, 0);
        }
    ...
    if (blockType->blockModel == 6) {
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                             Vector3(0, 8, 16), Vector3(0, 8, 0), Vector3(0, 0, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 8, 16),
                             Vector3(0, 16, 16), Vector3(0, 16, 8), Vector3(0, 8, 8));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                             Vector3(16, 8, 0), Vector3(16, 8, 16), Vector3(16, 0, 16));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 8, 8),
                             Vector3(16, 16, 8), Vector3(16, 16, 16), Vector3(16, 8, 16));
        ...
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 8, 16),
                             Vector3(16, 8, 16), Vector3(16, 8, 0), Vector3(0, 8, 0), 1, 0);
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 16, 16),
                             Vector3(16, 16, 16), Vector3(16, 16, 8), Vector3(0, 16, 8));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 8, 0),
                             Vector3(16, 8, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 16, 8),
                             Vector3(16, 16, 8), Vector3(16, 8, 8), Vector3(0, 8, 8), 1, 0);

        if (BlockTransparentOrSpecial(x, y, z + 1) == true)
            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6,
    ...
    if (blockType->blockModel == 7) {
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                             Vector3(0, 8, 16), Vector3(0, 8, 0), Vector3(0, 0, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 8, 8),
                             Vector3(0, 16, 8), Vector3(0, 16, 0), Vector3(0, 8, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                             Vector3(16, 8, 0), Vector3(16, 8, 16), Vector3(16, 0, 16));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 8, 0),
                             Vector3(16, 16, 0), Vector3(16, 16, 8), Vector3(16, 8, 8));
        ...
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 8, 16),
                             Vector3(16, 8, 16), Vector3(16, 8, 0), Vector3(0, 8, 0), 1, 0);
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 16, 8),
                             Vector3(16, 16, 8), Vector3(16, 16, 0), Vector3(0, 16, 0));

        if (BlockTransparentOrSpecial(x, y, z - 1) == true)
            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5,
                                        Vector3(0, 16, 0), Vector3(16, 16, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));

        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                             Vector3(16, 0, 16), Vector3(16, 8, 16), Vector3(0, 8, 16));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 8, 8),
                             Vector3(16, 8, 8), Vector3(16, 16, 8), Vector3(0, 16, 8), 1, 0);
    ...
    if (blockType->blockModel == 8) {
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(0, 0, 16),
                             Vector3(0, 8, 16), Vector3(0, 8, 0), Vector3(0, 0, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1, Vector3(8, 8, 16),
                             Vector3(8, 16, 16), Vector3(8, 16, 0), Vector3(8, 8, 0), 1, 0);

        if (BlockTransparentOrSpecial(x + 1, y, z) == true)
            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2,
                                        Vector3(16, 0, 0), Vector3(16, 16, 0), Vector3(16, 16, 16), Vector3(16, 0, 16));
        ...
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 8, 16),
                             Vector3(16, 8, 16), Vector3(16, 8, 0), Vector3(0, 8, 0), 1, 0);
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(8, 16, 16),
                             Vector3(16, 16, 16), Vector3(16, 16, 0), Vector3(8, 16, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 8, 0),
                             Vector3(16, 8, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(8, 16, 0),
                             Vector3(16, 16, 0), Vector3(16, 8, 0), Vector3(8, 8, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                             Vector3(16, 0, 16), Vector3(16, 8, 16), Vector3(0, 8, 16));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(8, 8, 16),
                             Vector3(16, 8, 16), Vector3(16, 16, 16), Vector3(8, 16, 16));
    ...
    if (blockType->blockModel == 9) {
        if (BlockTransparentOrSpecial(x - 1, y, z) == true)
            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1,
                                        Vector3(0, 0, 16), Vector3(0, 16, 16), Vector3(0, 16, 0), Vector3(0, 0, 0));

        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(16, 0, 0),
                             Vector3(16, 8, 0), Vector3(16, 8, 16), Vector3(16, 0, 16));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(8, 8, 0),
                             Vector3(8, 16, 0), Vector3(8, 16, 16), Vector3(8, 8, 16), 1, 0);
        ...
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 8, 16),
                             Vector3(16, 8, 16), Vector3(16, 8, 0), Vector3(0, 8, 0), 1, 0);
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 16, 16),
                             Vector3(8, 16, 16), Vector3(8, 16, 0), Vector3(0, 16, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 8, 0),
                             Vector3(16, 8, 0), Vector3(16, 0, 0), Vector3(0, 0, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 16, 0),
                             Vector3(8, 16, 0), Vector3(8, 8, 0), Vector3(0, 8, 0));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 16),
                             Vector3(16, 0, 16), Vector3(16, 8, 16), Vector3(0, 8, 16));
        BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 8, 16),
                             Vector3(8, 8, 16), Vector3(8, 16, 16), Vector3(0, 16, 16));
    ...
    if (blockType->blockModel == 10) {
        ...
        connectLeft = (BlockSolid(x - 1, y, z) &&
                       (!BlockTransparentOrSpecial(x - 1, y, z) || GetBlock(x - 1, y, z) == Block));
        connectRight = (BlockSolid(x + 1, y, z) &&
                        (!BlockTransparentOrSpecial(x + 1, y, z) || GetBlock(x + 1, y, z) == Block));
        connectBack = (BlockSolid(x, y, z - 1) &&
                       (!BlockTransparentOrSpecial(x, y, z - 1) || GetBlock(x, y, z - 1) == Block));
        connectFront = (BlockSolid(x, y, z + 1) &&
                        (!BlockTransparentOrSpecial(x, y, z + 1) || GetBlock(x, y, z + 1) == Block));
        int connectsNum = 0;

        if (!connectFront && !connectBack && !connectLeft && !connectRight) {
            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1,
                                        Vector3(8, 0, 16), Vector3(8, 16, 16), Vector3(8, 16, 0), Vector3(8, 0, 0));
            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2, Vector3(8, 0, 0),
                                 Vector3(8, 16, 0), Vector3(8, 16, 16), Vector3(8, 0, 16), true, false, 0);

            if (BlockSolid(x, y - 1, z) == false) {
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 0, 7),
                                     Vector3(16, 0, 7), Vector3(16, 0, 9), Vector3(0, 0, 9));
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(7, 0, 0),
                                     Vector3(9, 0, 0), Vector3(9, 0, 16), Vector3(7, 0, 16));

                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 0, 16),
                                     Vector3(9, 0, 16), Vector3(9, 0, 0), Vector3(7, 0, 0), 1, 0);
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 0, 9),
                                     Vector3(16, 0, 9), Vector3(16, 0, 7), Vector3(0, 0, 7), 1, 0);
            }
            if (BlockSolid(x, y + 1, z) == false) {
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(7, 16, 16),
                                     Vector3(9, 16, 16), Vector3(9, 16, 0), Vector3(7, 16, 0), 1, 0);
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4, Vector3(0, 16, 9),
                                     Vector3(16, 16, 9), Vector3(16, 16, 7), Vector3(0, 16, 7), 1, 0);

                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(0, 16, 7),
                                     Vector3(16, 16, 7), Vector3(16, 16, 9), Vector3(0, 16, 9));
                BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3, Vector3(7, 16, 0),
                                     Vector3(9, 16, 0), Vector3(9, 16, 16), Vector3(7, 16, 16));
            }

            BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5, Vector3(0, 16, 8),
                                 Vector3(16, 16, 8), Vector3(16, 0, 8), Vector3(0, 0, 8), true, false, 5);
            BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6, Vector3(0, 0, 8),
                                        Vector3(16, 0, 8), Vector3(16, 16, 8), Vector3(0, 16, 8));
        } else {
            if (connectLeft) {
                BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5,
                                            Vector3(0, 16, 8), Vector3(8, 16, 8), Vector3(8, 0, 8), Vector3(0, 0, 8));
                BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6,
                                            Vector3(0, 0, 8), Vector3(8, 0, 8), Vector3(8, 16, 8), Vector3(0, 16, 8));

                if (BlockSolid(x, y - 1, z) == false) {
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3,
                                         Vector3(0, 0, 7), Vector3(8, 0, 7), Vector3(8, 0, 9), Vector3(0, 0, 9));
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4,
                                         Vector3(0, 0, 9), Vector3(8, 0, 9), Vector3(8, 0, 7), Vector3(0, 0, 7), 1, 0);
                }
                if (BlockSolid(x, y + 1, z) == false) {
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4,
                                         Vector3(0, 16, 9), Vector3(8, 16, 9), Vector3(8, 16, 7), Vector3(0, 16, 7), 1,
                                         0);
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3,
                                         Vector3(0, 16, 7), Vector3(8, 16, 7), Vector3(8, 16, 9), Vector3(0, 16, 9));
                }
                connectsNum++;
            }
            if (connectRight) {
                BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5,
                                            Vector3(8, 16, 8), Vector3(16, 16, 8), Vector3(16, 0, 8), Vector3(8, 0, 8));
                BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6,
                                            Vector3(8, 0, 8), Vector3(16, 0, 8), Vector3(16, 16, 8), Vector3(8, 16, 8));

                if (BlockSolid(x, y - 1, z) == false) {
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3,
                                         Vector3(8, 0, 7), Vector3(16, 0, 7), Vector3(16, 0, 9), Vector3(8, 0, 9));
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4,
                                         Vector3(8, 0, 9), Vector3(16, 0, 9), Vector3(16, 0, 7), Vector3(8, 0, 7), 1,
                                         0);
                }
                if (BlockSolid(x, y + 1, z) == false) {
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4,
                                         Vector3(8, 16, 9), Vector3(16, 16, 9), Vector3(16, 16, 7), Vector3(8, 16, 7),
                                         1, 0);
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3,
                                         Vector3(8, 16, 7), Vector3(16, 16, 7), Vector3(16, 16, 9), Vector3(8, 16, 9));
                }
                connectsNum++;
            }
            if (connectBack) {
                BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1,
                                            Vector3(8, 0, 8), Vector3(8, 16, 8), Vector3(8, 16, 0), Vector3(8, 0, 0));
                BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2,
                                            Vector3(8, 0, 0), Vector3(8, 16, 0), Vector3(8, 16, 8), Vector3(8, 0, 8));

                if (BlockSolid(x, y - 1, z) == false) {
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3,
                                         Vector3(7, 0, 0), Vector3(9, 0, 0), Vector3(9, 0, 8), Vector3(7, 0, 8));
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4,
                                         Vector3(7, 0, 8), Vector3(9, 0, 8), Vector3(9, 0, 0), Vector3(7, 0, 0));
                }
                if (BlockSolid(x, y + 1, z) == false) {
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4,
                                         Vector3(7, 16, 8), Vector3(9, 16, 8), Vector3(9, 16, 0), Vector3(7, 16, 0));
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3,
                                         Vector3(7, 16, 0), Vector3(9, 16, 0), Vector3(9, 16, 8), Vector3(7, 16, 8));
                }
                connectsNum++;
            }
            if (connectFront) {
                BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1,
                                            Vector3(8, 0, 16), Vector3(8, 16, 16), Vector3(8, 16, 8), Vector3(8, 0, 8));
                BuildWorldBlockPlaneNoCheck(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2,
                                            Vector3(8, 0, 8), Vector3(8, 16, 8), Vector3(8, 16, 16), Vector3(8, 0, 16));

                if (BlockSolid(x, y - 1, z) == false) {
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3,
                                         Vector3(7, 0, 8), Vector3(9, 0, 8), Vector3(9, 0, 16), Vector3(7, 0, 16));
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4,
                                         Vector3(7, 0, 16), Vector3(9, 0, 16), Vector3(9, 0, 8), Vector3(7, 0, 8));
                }
                if (BlockSolid(x, y + 1, z) == false) {
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 4,
                                         Vector3(7, 16, 16), Vector3(9, 16, 16), Vector3(9, 16, 8), Vector3(7, 16, 8));
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 3,
                                         Vector3(7, 16, 8), Vector3(9, 16, 8), Vector3(9, 16, 16), Vector3(7, 16, 16));
                }
                connectsNum++;
            }
            ...
            if (connectsNum == 1) {
                if (connectLeft || connectRight) {
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 1,
                                         Vector3(8, 0, 9), Vector3(8, 16, 9), Vector3(8, 16, 7), Vector3(8, 0, 7), true,
                                         false, 0);
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 2,
                                         Vector3(8, 0, 7), Vector3(8, 16, 7), Vector3(8, 16, 9), Vector3(8, 0, 9), true,
                                         false, 0);
                }
                if (connectBack || connectFront) {
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 5,
                                         Vector3(7, 16, 8), Vector3(9, 16, 8), Vector3(9, 0, 8), Vector3(7, 0, 8), true,
                                         false, 0);
                    BuildWorldBlockPlane(blockType, x, y, z, iVertex, MeshChunk, Block, transparent, 6,
                                         Vector3(7, 0, 8), Vector3(9, 0, 8), Vector3(9, 16, 8), Vector3(7, 16, 8), true,
                                         false, 0);
                }
            }
        }
...
void CraftWorld::GetNormalBlock(int x, int y, int z, int &iVertex, SimpleMeshChunk *MeshChunk, block_t Block,
    ...
    if (transparent == true) {
        if ((transparentBlock == true && Block1 != Block) || Block1 == 0 ||
            (specialBlock > 0 && Block1 != 4 && Block1 != 112)) {
            canCreate = true;
        }
    ...
    if (canCreate == true) {
        ...
        if (mainOptions.smoothLighting == true) {
            ...
            if (onCorners) {
                if (!BlockTransparentOrLightSource(x - 1, y + 1, z) ||
                    !BlockTransparentOrLightSource(x - 1, y, z + 1) ||
                    !BlockTransparentOrLightSource(x - 1, y + 1, z + 1)) {
                ...
                if (!BlockTransparentOrLightSourceNoCheck(x - 1, y + 1, z) ||
                    !BlockTransparentOrLightSourceNoCheck(x - 1, y, z + 1) ||
                    !BlockTransparentOrLightSourceNoCheck(x - 1, y + 1, z + 1)) {
                    light1noV -= lightFactor1;
                }
    ...
    if (canCreate) {
        ...
        if (mainOptions.smoothLighting == true) {
            ...
            if (onCorners) {
                if (!BlockTransparentOrLightSource(x - 1, y - 1, z) ||
                    !BlockTransparentOrLightSource(x - 1, y - 1, z - 1) ||
                    !BlockTransparentOrLightSource(x, y - 1, z - 1)) {
                    light1noV -= lightFactor1;
                }

                if (!BlockTransparentOrLightSource(x, y - 1, z - 1) ||
                    !BlockTransparentOrLightSource(x + 1, y - 1, z - 1) ||
                    !BlockTransparentOrLightSource(x + 1, y - 1, z)) {
                    light2noV -= lightFactor2;
                }
                ...
                if (!BlockTransparentOrLightSourceNoCheck(x - 1, y - 1, z) ||
                    !BlockTransparentOrLightSourceNoCheck(x - 1, y - 1, z - 1) ||
                    !BlockTransparentOrLightSourceNoCheck(x, y - 1, z - 1)) {
                    light1noV -= lightFactor1;
                }

                if (!BlockTransparentOrLightSourceNoCheck(x, y - 1, z - 1) ||
                    !BlockTransparentOrLightSourceNoCheck(x + 1, y - 1, z - 1) ||
                    !BlockTransparentOrLightSourceNoCheck(x + 1, y - 1, z)) {
                    light2noV -= lightFactor2;
                }
...
void CraftWorld::rebuildNearestChunksForLight(int id, Vector3 pos) {
    ...
    if (posInChunk.x < 10) {
        ...
        if (posInChunk.z < 10) {
            ...
            if (posInChunk.y < 10) {
                chunkNumber = getChunkId(
                        Vector3(temp.x - CHUNK_SIZE / 2, temp.y - CHUNK_SIZE / 2, temp.z - CHUNK_SIZE / 2));
                if (chunkNumber != -1) {
                    AddChunkToUpdate(chunkNumber);
                }
            }
        ...
        for (int y = StartY; y < CHUNK_SIZE + StartY; ++y) {
            for (int x = StartX; x < CHUNK_SIZE + StartX; ++x) {
                ...
                if (block == GrassBlock::getID() &&
                    (GetShadowLevel(x, y, z) > 6 || BlockTransparentOrSpecial(x, y + 1, z) == false)) {
                ...
                if (block == CaneBlock::getID() && GetBlock(x, y + 1, z) == 0 && GetBlock(x, y - 3, z) != 33 &&
                    GetBlock(x, y - 3, z) != 0) {
                    if (rand() % 100 > 92) {
                        GetBlock(x, y + 1, z) = 33;
                        continue;
                ...
                if (block == Lava::getID() && GetBlock(x + 2, y, z) == WaterBlock::getID() &&
                    GetBlock(x + 1, y, z) == 0) {
                ...
                if (block == Lava::getID() && GetBlock(x, y, z + 2) == WaterBlock::getID() &&
                    GetBlock(x, y, z + 1) == 0) {
                ...
                if (x > 0 && x < WORLD_SIZE && y > 0 && y < WORLD_SIZE && z > 0 && z < WORLD_SIZE) {
                    if (block == Soil::getID() || block == WateredSoil::getID()) {
                        if (water_x != -1 && water_z != -1) {
                            BoundingBox hitBox = BoundingBox(Vector3(water_x - 2, water_y - 1, water_z - 2),
                                                             Vector3(water_x + 2, water_y + 1, water_z + 2));
                    ...
                    if (block == PumpkinPlant4::getID()) {
                        somethingWasUpdated = true;
                        bool canGrow = true;
                        if (GetBlock(x - 1, y, z) == Pumpkin1::getID() || GetBlock(x + 1, y, z) == Pumpkin1::getID() ||
                            GetBlock(x, y, z - 1) == Pumpkin1::getID() || GetBlock(x, y, z + 1) == Pumpkin1::getID()) {
                            canGrow = false;
                        }
                        if (rand() % 7 == 0 && canGrow) {
                            switch (rand() % 4) {
                    ...
                    if (block == 121 && (GetBlock(x, y - 1, z) == 1 || GetBlock(x, y - 1, z) == 2)) {
                        somethingWasUpdated = true;
                        if ((rand() % 20 == 0 && (GetBlock(x, y - 1, z) == 1 || GetBlock(x, y - 1, z) == 2)) ||
                            (rand() % 32 == 0 && GetBlock(x, y - 1, z) == 29)) {
                    ...
                    if (block == 123 && (GetBlock(x, y - 1, z) == 1 || GetBlock(x, y - 1, z) == 2)) {
                        somethingWasUpdated = true;
                        if ((rand() % 22 == 0 && (GetBlock(x, y - 1, z) == 1 || GetBlock(x, y - 1, z) == 2)) ||
                            (rand() % 40 == 0 && GetBlock(x, y - 1, z) == 29)) {
                            mainStatistics.treesGrowned += 1;
                            int height = 2 + rand() % 2;
                    ...
                    /// spruce tree growing
                    if (block == 122 &&
                        (GetBlock(x, y - 1, z) == 1 || GetBlock(x, y - 1, z) == 2 || GetBlock(x, y - 1, z) == 29)) {
                        somethingWasUpdated = true;
                        if ((rand() % 22 < 2 && (GetBlock(x, y - 1, z) == 1 || GetBlock(x, y - 1, z) == 2)) ||
                            (rand() % 19 == 0 && GetBlock(x, y - 1, z) == 29)) {
                            mainStatistics.treesGrowned += 1;
...
int CraftWorld::getChunkId(Vector3 pos) {
    ...
    int id = floorf(pos.x / CHUNK_SIZE) + floorf(pos.y / CHUNK_SIZE) * (WORLD_SIZE / CHUNK_SIZE) +
             floorf(pos.z / CHUNK_SIZE) * (WORLD_SIZE / CHUNK_SIZE) * (WORLD_HEIGHT / CHUNK_SIZE);
    return id;
}
...
void CraftWorld::drawCubes(int i, float light) {
    ...
    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, blockTypes[i].pointsLeftRight,
                    0, blockTypes[i].verticesLeftRight);

    sceGuColor(GU_COLOR(light * 0.71, light * 0.71, light * 0.71, 1));
    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, blockTypes[i].pointsBackFront,
                    0, blockTypes[i].verticesBackFront);

    sceGuColor(GU_COLOR(light, light, light, 1));
    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, blockTypes[i].pointsBottomTop,
                    0, blockTypes[i].verticesBottomTop);
...
void CraftWorld::drawHand() {
    ...
    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, 36, 0, HumanoidModel::armVertices);
...
void CraftWorld::drawClouds() {
    ...
    DrawSetCulling(true);
    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, 600, 0,
                    blockTypes[141].verticesBottomTop);
    DrawSetCulling(false);
    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, 600, 0,
                    blockTypes[141].verticesBottomTop);
...
void CraftWorld::drawItems(int i) {
    Item* item = ItemTypes::getItem(i);
    DrawPlaceTexture(true);
    sceGuDisable(GU_DEPTH_TEST);
    DrawSetDepthMask(true);
    DrawSetBlend(true);
    
    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, item->points, 0,
                    item->vertices);
...
void CraftWorld::drawHandItems(int i, float light) {
    ...
    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, item->addPoints,
                    0, item->addVertices);

    DrawSetCulling(true);
    sceGuColor(GU_COLOR(light * 0.75, light * 0.75, light * 0.75, 1));
    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, item->add2Points,
                    0, item->add2Vertices);
    sceGuColor(GU_COLOR(light, light, light, 1));
    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, item->points, 0,
                    item->vertices);
...
void CraftWorld::drawShoulder(int chestplateId, float light) {
    ...
    Item* item = ItemTypes::getItem(chestplateId);
    if (item->hasFeature(FeatureType::armor)) {
        ArmorFeature* armorFeature = static_cast<ArmorFeature*>(item->getFeature(FeatureType::armor));
        armor_slot armorSlot = armorFeature->getArmorSlot();
...
void CraftWorld::drawBelly(int leggingsId, float light) {
    ...
    Item* item = ItemTypes::getItem(leggingsId);
    if (item->hasFeature(FeatureType::armor)) {
        ArmorFeature* armorFeature = static_cast<ArmorFeature*>(item->getFeature(FeatureType::armor));
        armor_slot armorSlot = armorFeature->getArmorSlot();
...
void CraftWorld::drawWorld(Frustum &camFrustum, bool camUpdate) {
    ...
    if (mNearestChunksIds.empty() == false) {
        for (unsigned int i2 = 0; i2 != mNearestChunksIds.size(); i2++) {
            ...
            if (playerZoneBB.intersect(mChunks[i]->bBox)) {
                if (mChunks[i]->trienglesCount > 0 || mTransparentChunks[i]->trienglesCount > 0 ||
                    !mChunks[i]->created) {
                    if (camFrustum.BoxInFrustum(mChunks[i]->bBox) == Frustum::Intersects) {
                        ...
                        if ((!mChunks[i]->created || mChunks[i]->needUpdate) && chunksCreatedInFrameCount == 0 &&
                            !found) {
                            int distance = FastDistance2d(abs(playerPos.x - (mChunks[i]->chunkStartX + 7)),
                                                          abs(playerPos.z - (mChunks[i]->chunkStartZ + 7))) +
                                           abs(playerPos.y - (mChunks[i]->chunkStartY + 7)) * 0.85f;
...
void CraftWorld::UpdatePlayerZoneBB(Vector3 playerPosition) {
    ...
    playerZoneBB = BoundingBox(Vector3(playerPosition.x - playerZoneSize.x, playerPosition.y - playerZoneSize.y,
                                       playerPosition.z - playerZoneSize.z),
                               Vector3(playerPosition.x + playerZoneSize.x, playerPosition.y + playerZoneSize.y,
                                       playerPosition.z + playerZoneSize.z));
}
        ...
        for (int i = 0; i < frameSize; i++) {
            for (int j = 0; j < frameSize; j++) {
                TextureManager::Instance()->setColour(frameSize * 6 + i,
                                                      frameSize * 4 + j,
                                                      TextureManager::Instance()->getRed(i,
                                                                                         lastClockFrame * frameSize + j,
                                                                                         textureClockId),
                                                      TextureManager::Instance()->getGreen(i,
                                                                                           lastClockFrame * frameSize +
                                                                                           j, textureClockId),
                                                      TextureManager::Instance()->getBlue(i,
                                                                                          lastClockFrame * frameSize +
                                                                                          j, textureClockId),
                                                      TextureManager::Instance()->getAlpha(i,
                                                                                           lastClockFrame * frameSize +
                                                                                           j, textureClockId),
                                                      textureItemsId);
            }
...
void CraftWorld::UpdateCompassTexture(float cameraAngle) {
    float angle = ATAN2(playerPos.z - playerSpawnPointPosition.z, playerPos.x - playerSpawnPointPosition.x) -
                  (cameraAngle - PI);
    if (angle < -PI) {
        angle = angle + 2 * PI;
    }
        ...
        for (int i = 0; i < frameSize; i++) {
            for (int j = 0; j < frameSize; j++) {
                TextureManager::Instance()->setColour(frameSize * 6 + i,
                                                      frameSize * 3 + j,
                                                      TextureManager::Instance()->getRed(i,
                                                                                         lastCompassFrame * frameSize +
                                                                                         j, textureCompassId),
                                                      TextureManager::Instance()->getGreen(i, lastCompassFrame *
                                                                                              frameSize + j,
                                                                                           textureCompassId),
                                                      TextureManager::Instance()->getBlue(i,
                                                                                          lastCompassFrame * frameSize +
                                                                                          j, textureCompassId),
                                                      TextureManager::Instance()->getAlpha(i, lastCompassFrame *
                                                                                              frameSize + j,
                                                                                           textureCompassId),
                                                      textureItemsId);
            }
...
void CraftWorld::UpdateWorldAnimation(float dt) {
    if (mainOptions.worldBlockAnimation == true) {
        ...
        if (animationTimer >= animationTimerEnd) // every 0.075 secs
        {
            ...
            if (animationWaterFrame >= TextureManager::Instance()->getHeight(textureWaterAnimationId) /
                                       TextureManager::Instance()->getWidth(
                                               textureWaterAnimationId)) // if all frames passed
            {
                animationWaterFrame = 0; // zeroize frames
            }

            if (animationLavaFrame >= TextureManager::Instance()->getHeight(textureLavaAnimationId) /
                                      TextureManager::Instance()->getWidth(textureLavaAnimationId) - 1) {
            ...
            for (int i = 0; i < waterFrameSize; i++) {
                for (int j = 0; j < waterFrameSize; j++) {
                    ...
                    TextureManager::Instance()->setColour(waterFrameSize * 14 + i,
                                                          waterFrameSize * 15 + j,
                                                          TextureManager::Instance()->getRed(i, animationWaterFrame *
                                                                                                waterFrameSize + j,
                                                                                             textureWaterAnimationId),
                                                          TextureManager::Instance()->getGreen(i, animationWaterFrame *
                                                                                                  waterFrameSize + j,
                                                                                               textureWaterAnimationId),
                                                          TextureManager::Instance()->getBlue(i, animationWaterFrame *
                                                                                                 waterFrameSize + j,
                                                                                              textureWaterAnimationId),
                                                          184,
                                                          textureTerrainId);
                    ...
                    TextureManager::Instance()->setColour(waterFrameSize * 15 + i,
                                                          waterFrameSize * 15 + j,
                                                          TextureManager::Instance()->getRed(i, animationLavaFrame *
                                                                                                waterFrameSize + j,
                                                                                             textureLavaAnimationId),
                                                          TextureManager::Instance()->getGreen(i, animationLavaFrame *
                                                                                                  waterFrameSize + j,
                                                                                               textureLavaAnimationId),
                                                          TextureManager::Instance()->getBlue(i, animationLavaFrame *
                                                                                                 waterFrameSize + j,
                                                                                              textureLavaAnimationId),
                                                          255,
                                                          textureTerrainId);
    ...
    if (spawnZombiesTimer > 15.0f) {
        ...
        if ((worldDayTime > 21 || worldDayTime < 6) && mainOptions.difficulty != 0 && playerPos.y > 55.0f) // spawn zombs
        {
            if (rand() % 50 <= 26 + (24 * mZombies.empty()) && mZombies.size() < 8) {
                ...
                {
                    if (spawnY != -1 && spawnY < WORLD_HEIGHT && spawnX < WORLD_SIZE && spawnZ < WORLD_SIZE &&
                        spawnY > 0 && spawnX > 0 && spawnZ > 0) {
                        SpawnZombie(spawnX + 0.5f, spawnY + 1.8f, spawnZ + 0.5f);
                        SpawnZombie(spawnX + 2.5f, groundHeightWater(spawnX + 2.0f, spawnZ + 2.0f) + 1.8f,
                                    spawnZ + 2.5f);
                        wasSpawned = true;
                    }
                }
                        ...
                        if (spawnY != -1) {
                            if (GetBlock(spawnX, spawnY + 1, spawnZ) == 0) {
                                if (BlockFinalLight(spawnX, spawnY, spawnZ) <= 0.15f) {
                                    if (spawnY != -1 && spawnY < WORLD_HEIGHT && spawnX < WORLD_SIZE &&
                                        spawnZ < WORLD_SIZE && spawnY > 0 && spawnX > 0 && spawnZ > 0) {
                                        SpawnZombie(spawnX + 0.5f, spawnY + 1.8f, spawnZ + 0.5f);
                                        if (rand() % 5 == 0) {
                                            SpawnZombie(spawnX + 0.5f, spawnY + 1.8f, spawnZ + 0.5f);
                                        }
            ...
            if (rand() % 50 <= 27 - mCreepers.size() * 1.5f && mCreepers.size() < 4) {
                ...
                if (BlockFinalLight(spawnX, spawnY, spawnZ) < 0.25f) // spawn only at black areas
                {
                    if (spawnY != -1 && spawnY < WORLD_HEIGHT && spawnX < WORLD_SIZE && spawnZ < WORLD_SIZE &&
                        ...
                        if (rand() % 4 == 0) {
                            SpawnCreeper(spawnX + 2.5f, groundHeightWater(spawnX + 2.0f, spawnZ + 2.0f) + 1.8f,
                                         spawnZ + 2.5f);
                        }
                    }
                }
    ...
    if (spawnCowsTimer < 0.0f) {
        if (worldDayTime > 6 && worldDayTime < 21) {
            if (rand() % 2 == 0 && mCows.size() + mSheeps.size() < 12) {
                ...
                if (spawnY != -1 && spawnY < WORLD_HEIGHT && spawnX < WORLD_SIZE && spawnZ < WORLD_SIZE && spawnY > 0 &&
                    spawnX > 0 && spawnZ > 0) {
...
bool CraftWorld::PlayerOnHalfBlock(Vector3 pos) {
    ...
    if (blockTypes[posBlockID].blockModel == 1 ||
        ...
        (blockTypes[posBlockID].blockModel >= 6 && blockTypes[posBlockID].blockModel <= 9 &&
...
void CraftWorld::BuildExplodeSphere(float radius, int X, int Y, int Z) {
            ...
            if (x > X) {
                for (int x2 = x; x2 >= X - (x2 - X); x2--) {
                    ...
                    if (block == TNTBlock::getID()) {
                        ...
                        SpawnTNTentity(((int) testPos1.x) + 0.5f, ((int) testPos1.y) + 0.5f, ((int) testPos1.z) + 0.5f,
                                       0.25f + (rand() % 11) * 0.25f);
    ...
    if (dropIDs.size() > 1) {
        ...
        for (unsigned int i = 0; i < dropIDs.size(); i++) {
            if (dropIDs[i] != temp) {
                DropThis(temp, num,
                         Vector3(X - 1.5f + (rand() % 7) * 0.5f, Y - radius + 1.5f, Z - 1.5f + (rand() % 7) * 0.5));
                num = 0;
            }
...
void CraftWorld::BuildWorldBlockPlaneNoCheck(BaseBlock *blockType, int x, int y, int z, int &iVertex,
                                             SimpleMeshChunk *MeshChunk, block_t Block, bool transparent, char side,
...
void
CraftWorld::BuildWorldBlockPlane(BaseBlock *blockType, int x, int y, int z, int &iVertex, SimpleMeshChunk *MeshChunk,
    ...
    if (side == 1) {
        ...
        if (transparent == true) {
            if ((transparentBlock == true && Block1 != Block) || Block1 == 0 ||
                (specialBlock > 0 && Block1 != 4 && Block1 != 112 && Block1 != 9 && Block1 != 37 && Block1 != 38)) {
                canCreate = true;
            }
            if (transparentBlock == false && specialBlock == 0) {
                canCreate = false;
        ...
        if (canCreate == true) {
            ...
            MeshChunk->vert(x + point1.x * pixel, y + point1.y * pixel, z + point1.z * pixel, right, down, light1.x,
                            light1.y, light1.z);
            MeshChunk->vert(x + point2.x * pixel, y + point2.y * pixel, z + point2.z * pixel, right, up, light2.x,
                            light2.y, light2.z);
            MeshChunk->vert(x + point3.x * pixel, y + point3.y * pixel, z + point3.z * pixel, left, up, light4.x,
                            light4.y, light4.z);
            MeshChunk->vert(x + point4.x * pixel, y + point4.y * pixel, z + point4.z * pixel, left, down, light3.x,
                            light3.y, light3.z);
    ...
    if (side == 2) {
        ...
        if (canCreate) {
            ...
            MeshChunk->vert(x + point1.x * pixel, y + point1.y * pixel, z + point1.z * pixel, right, down, light3.x,
                            light3.y, light3.z);
            MeshChunk->vert(x + point2.x * pixel, y + point2.y * pixel, z + point2.z * pixel, right, up, light4.x,
                            light4.y, light4.z);
            MeshChunk->vert(x + point3.x * pixel, y + point3.y * pixel, z + point3.z * pixel, left, up, light2.x,
                            light2.y, light2.z);
            MeshChunk->vert(x + point4.x * pixel, y + point4.y * pixel, z + point4.z * pixel, left, down, light1.x,
                            light1.y, light1.z);
    ...
    if (side == 3) {
        ...
        if (canCreate) {
            ...
            MeshChunk->vert(x + point1.x * pixel, y + point1.y * pixel, z + point1.z * pixel, left, up, light1.x,
                            light1.y, light1.z);
            MeshChunk->vert(x + point2.x * pixel, y + point2.y * pixel, z + point2.z * pixel, right, up, light2.x,
                            light2.y, light2.z);
            MeshChunk->vert(x + point3.x * pixel, y + point3.y * pixel, z + point3.z * pixel, right, down, light3.x,
                            light3.y, light3.z);
            MeshChunk->vert(x + point4.x * pixel, y + point4.y * pixel, z + point4.z * pixel, left, down, light4.x,
                            light4.y, light4.z);
    ...
    if (side == 5) {
        ...
        if (canCreate) {
            ...
            MeshChunk->vert(x + point1.x * pixel, y + point1.y * pixel, z + point1.z * pixel, right, up, light2.x,
                            light2.y, light2.z);
            MeshChunk->vert(x + point2.x * pixel, y + point2.y * pixel, z + point2.z * pixel, left, up, light4.x,
                            light4.y, light4.z);
            MeshChunk->vert(x + point3.x * pixel, y + point3.y * pixel, z + point3.z * pixel, left, down, light3.x,
                            light3.y, light3.z);
            MeshChunk->vert(x + point4.x * pixel, y + point4.y * pixel, z + point4.z * pixel, right, down, light1.x,
                            light1.y, light1.z);
    ...
    if (side == 6) {
        ...
        if (canCreate == true) {
            ...
            MeshChunk->vert(x + point1.x * pixel, y + point1.y * pixel, z + point1.z * pixel, left, down, light1.x,
                            light1.y, light1.z);
            MeshChunk->vert(x + point2.x * pixel, y + point2.y * pixel, z + point2.z * pixel, right, down, light3.x,
                            light3.y, light3.z);
            MeshChunk->vert(x + point3.x * pixel, y + point3.y * pixel, z + point3.z * pixel, right, up, light4.x,
                            light4.y, light4.z);
            MeshChunk->vert(x + point4.x * pixel, y + point4.y * pixel, z + point4.z * pixel, left, up, light2.x,
                            light2.y, light2.z);
        ...
        if (!alwaysBuild) {
            ...
            if (transparent) {
                ...
                if (x > 0) {
                    ...
                    Block1 = GetBlock(x - 1, y, z);
                    if ((transparentBlock == true && Block1 != Block) || Block1 == 0 ||
                        (BlockSpecial(x - 1, y, z) && Block1 != 9 && Block1 != 4 && Block1 != 112 && Block1 != 38))
                        canCreate = true;
                }
            } else {
                if (x > 0) {
        ...
        if (!alwaysBuild) {
            ...
            if (transparent) {
                ...
                if (y > 0) {
                    ...
                    Block1 = GetBlock(x, y - 1, z);
                    if ((transparentBlock == true && Block1 != Block) || Block1 == 0 ||
                        (BlockSpecial(x, y - 1, z) && Block1 != 9 && Block1 != 4 && Block1 != 112 && Block1 != 38))
                        canCreate = true;
                }
            } else {
                if (y > 0) {
        ...
        if (!alwaysBuild) {
            ...
            if (transparent) {
                ...
                if (z > 0) {
                    ...
                    Block1 = GetBlock(x, y, z - 1);
                    if ((transparentBlock == true && Block1 != Block) || Block1 == 0 ||
                        (BlockSpecial(x, y, z - 1) && Block1 != 9 && Block1 != 4 && Block1 != 112 && Block1 != 38))
                        canCreate = true;
                }
            } else {
                if (z > 0) {
...
CraftWorld::BuildWorldBlockPlane(BaseBlock *blockType, int x, int y, int z, int &iVertex, SimpleMeshChunk *MeshChunk,
    ...
    switch (texturePlane) {
        case 0 :
            plane_x = blockType->leftPlane_x;
            plane_y = blockType->leftPlane_y;
            break;
        case 1 :
            plane_x = blockType->rightPlane_x;
            plane_y = blockType->rightPlane_y;
            break;
        case 2 :
            plane_x = blockType->downPlane_x;
            plane_y = blockType->downPlane_y;
            break;
        case 3 :
            plane_x = blockType->upPlane_x;
            plane_y = blockType->upPlane_y;
            break;
        case 4 :
            plane_x = blockType->botPlane_x;
            plane_y = blockType->botPlane_y;
            break;
        case 5 :
            plane_x = blockType->forPlane_x;
            plane_y = blockType->forPlane_y;
            break;
    }
...
void CraftWorld::BuildGUIBlockPlane(BaseBlock *blockType, char side, Vector3 point1, Vector3 point2, Vector3 point3,
                                    Vector3 point4) {
    float pixel, texturePixel;
