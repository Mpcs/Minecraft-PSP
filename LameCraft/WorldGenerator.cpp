...
void WorldGenerator::initRandompMap(int worldSize, int worldHeight, int chunkSize, CraftWorld *world, bool makeTrees,
                                    bool makeWater, bool makeCaves, unsigned int seedII, int worldType) {
    ...
    if (worldType == DEFAULT) {
        ...
        for (int z = 0; z < WORLD_SIZE; z++) {
            for (int x = 0; x < WORLD_SIZE; x++) {
                ...
                int Height = (InterpolateElevationNoise(x, z) +
                              InterpolateRoughnessNoise(x, z) * GetValue2D3(x, z, 6, 0.1f, 1)) * 64 * HillFrequency +
                             62;
                ...
                for (int j = 0; j < steps; j++) {
                    ...
                    for (int xx = x - radius; xx <= x + radius; xx++) {
                        for (int zz = z - radius; zz <= z + radius; zz++) {
                            for (int yy = y - radius; yy <= y + radius; yy++) {
                                if (world->GetBlock(xx, yy, zz) == IceBlock::getID() ||
                                    world->GetBlock(xx, yy, zz) == WaterBlock::getID()) {
                                    haveWater = true;
                                }
                            }
                        }
                    ...
                    if (haveWater == false) {
                        ...
                        if (rand() % 14 == 0) {
                            ...
                            world->BuildBlockSphere(radius, 0, x + radius * 1.5f, y, z + radius * 1.5f,
                                                    RockBlock::getID());
                            world->BuildBlockSphere(radius, 0, x + radius * 1.5f, y + radius * 1.5f, z + radius * 1.5f,
                                                    RockBlock::getID());
                            world->BuildBlockSphere(radius, 0, x, y - radius * 1.5f, z + radius * 1.5f,
                                                    RockBlock::getID());
                            world->BuildBlockSphere(radius, 0, x, y + radius * 1.5f, z + radius * 1.5f,
                                                    RockBlock::getID());
                        }
                    ...
                    reverseX == false ? x += world->LengthDirX((dataHor[j]), (dataVer[j])) * radius * 1.5f
                                      : x -= world->LengthDirX((dataHor[j]), (dataVer[j])) * radius * 1.5f;
                    reverseY == false ? y += world->LengthDirY((dataHor[j]), (dataVer[j])) * radius * 1.5f
                                      : y -= world->LengthDirY((dataHor[j]), (dataVer[j])) * radius * 1.5f;
                    reverseZ == false ? z += world->LengthDirZ((dataHor[j]), (dataVer[j])) * radius * 1.5f
                                      : z -= world->LengthDirZ((dataHor[j]), (dataVer[j])) * radius * 1.5f;
        ...
        for (int i = 0; i <= 1; i++) {
            ...
            world->SpawnCow(spawnX + 2.0f + 0.5f, world->groundHeightWater(spawnX + 2.0f, spawnZ + 2.0f) + 2.0f,
                            spawnZ + 2.0f + 0.5f);
        }
        ...
        for (int i = 0; i < 1; i++) {
            ...
            world->SpawnSheep(spawnX + 2.5f, world->groundHeightWater(spawnX + 2.0f, spawnZ + 2.0f) + 2.0f,
                              spawnZ + 2.5f);
        }
        ...
        while (tries < 3072) {
            ...
            newPlayerSpawnPosition = Vector3(border + rand() % (WORLD_SIZE - border - 1), 0,
                                             border + rand() % (WORLD_SIZE - border - 1));
            int groundBlock = world->GetBlock(newPlayerSpawnPosition.x,
                                              world->groundHeightWater(newPlayerSpawnPosition.x,
                                                                       newPlayerSpawnPosition.z),
                                              newPlayerSpawnPosition.z);
            if (groundBlock != WaterBlock::getID()) {
                if (groundBlock == Snow2::getID()) {
                    newPlayerSpawnPosition = Vector3(newPlayerSpawnPosition.x + 0.5f,
                                                     world->groundHeightWater(newPlayerSpawnPosition.x,
                                                                              newPlayerSpawnPosition.z) +
                                                     world->playerGrowth - 0.05f, newPlayerSpawnPosition.z + 0.5f);
                } else {
                    newPlayerSpawnPosition = Vector3(newPlayerSpawnPosition.x + 0.5f,
                                                     world->groundHeightWater(newPlayerSpawnPosition.x,
                                                                              newPlayerSpawnPosition.z) +
                                                     world->playerGrowth - 0.05f + 1, newPlayerSpawnPosition.z + 0.5f);
                }
                world->SetPlayerSpawnPosition(newPlayerSpawnPosition);
                break;
            }
            if (tries == 3071) {
                world->SetPlayerSpawnPosition(Vector3(WORLD_SIZE / 2 + 0.5f,
                                                      world->groundHeightWater(WORLD_SIZE / 2, WORLD_SIZE / 2) +
                                                      world->playerGrowth - 0.05f - 0.5f, WORLD_SIZE / 2 + 0.5f));
            }
            tries++;
        }
    ...
    if (worldType == SUPERFLAT) {
        ...
        world->SetPlayerSpawnPosition(Vector3(WORLD_SIZE / 2, world->groundHeight(WORLD_SIZE / 2, WORLD_SIZE / 2) +
                                                              world->playerGrowth - 0.05f + 1, WORLD_SIZE / 2));
    }
        ...
        for (int z = biomeCenterZ - biomeRadius; z <= biomeCenterZ + biomeRadius; z++) {
            for (int x = biomeCenterX - biomeRadius; x <= biomeCenterX + biomeRadius; x++) {
                if (world->FastDistance2d(abs(x - biomeCenterX), abs(z - biomeCenterZ)) < biomeRadius + 5) {
                    if (x == biomeCenterX - biomeRadius || x == biomeCenterX + biomeRadius ||
                        z == biomeCenterZ - biomeRadius || z == biomeCenterZ + biomeRadius) {
                        if (rand() % 4 == 1) {
                            continue;
                        }
                    }
                    ...
                    if (world->GetBlock(x, world->groundHeight(x, z), z) != 93 &&
                        world->GetBlock(x, world->groundHeight(x, z), z) != 32) {
        ...
        for (int z = biomeCenterZ - biomeRadius; z <= biomeCenterZ + biomeRadius; z++) {
            for (int x = biomeCenterX - biomeRadius; x <= biomeCenterX + biomeRadius; x++) {
                if (world->FastDistance2d(abs(x - biomeCenterX), abs(z - biomeCenterZ)) < biomeRadius + 5) {
                    ...
                    if (world->GetBlock(x, world->groundHeight(x, z), z) == 0) continue;
                    if (world->GetBlock(x, world->groundHeight(x, z), z) == 1 ||
                        world->GetBlock(x, world->groundHeight(x, z), z) == 2) {
    ...
    for (int z = 1; z < WORLD_SIZE - 1; z++) {
        for (int x = 1; x < WORLD_SIZE - 1; x++) {
            if (world->GetBlock(x, waterLevel, z) == SandBlock::getID()) {
                if (world->GetBlock(x - 1, waterLevel, z) == WaterBlock::getID() ||
                    world->GetBlock(x + 1, waterLevel, z) == WaterBlock::getID() ||
                    world->GetBlock(x, waterLevel, z - 1) == WaterBlock::getID() ||
                    world->GetBlock(x, waterLevel, z + 1) == WaterBlock::getID()) {
                    if (rand() % 48 == 0) {
                        int height = 1 + rand() % 3;
...
void WorldGenerator::GenerateClassicTree(int WORLD_SIZE, CraftWorld *world, int x, int y, int z, int trunkBlock,
                                         int leavesBlock) {
...
void WorldGenerator::initRandomTrees(int WORLD_SIZE, CraftWorld *world) {
    ...
    for (int i = 0; i < treesNum; i++) {
        ...
        bool snowTree = false;
        if (world->GetBlock(x, y, z) == Snow2::getID() &&
            (world->GetBlock(x, y - 1, z) == SnowSoil::getID() || world->GetBlock(x, y - 1, z) == GrassBlock::getID() ||
             world->GetBlock(x, y - 1, z) == DirtBlock::getID())) {
            snowTree = true;
        } else {
            snowTree = false;
...
void WorldGenerator::initTrees(int WORLD_SIZE, CraftWorld *world, PerlinModule *perlin, int seed) {
    ...
    for (int i = 0; i < treesNum; i++) {
        ...
        if (perlinHeight >= 72) {
            bool snowTree = false;
            if (world->GetBlock(x, y, z) == Snow2::getID() && (world->GetBlock(x, y - 1, z) == SnowSoil::getID() ||
                                                               world->GetBlock(x, y - 1, z) == GrassBlock::getID() ||
                                                               world->GetBlock(x, y - 1, z) == DirtBlock::getID())) {
                snowTree = true;
            } else {
                snowTree = false;
        ...
        for (int x2 = x - 3; x2 < x + 3; ++x2) {
            for (int z2 = z - 3; z2 < z + 3; ++z2) {
                if (x2 > 0 && x2 < WORLD_SIZE && z2 > 0 && z2 < WORLD_SIZE) {
                    ...
                    if (world->GetBlock(x2, world->groundHeight(x2, z2), z2) == GrassBlock::getID() ||
                        world->GetBlock(x2, world->groundHeight(x2, z2), z2) == DirtBlock::getID() ||
                        world->GetBlock(x2, world->groundHeight(x2, z2), z2) == SnowSoil::getID()) {
        ...
        for (int x = centerX - size; x <= centerX + size; x++) {
            for (int z = centerZ - size; z <= centerZ + size; z++) {
                ...
                if (world->GetBlock(x, y, z) == 1) {
                    if (x == centerX - size || x == centerX + size || z == centerZ - size ||
                        z == centerZ + size) // check for corner flower
                    {
                        if (rand() % 8 == 1) // 1/8
                        {
...
void WorldGenerator::initGrass(int WORLD_SIZE, CraftWorld *world) {
    ...
    for (int i = 0; i < NumTypes; ++i) {
        ...
        if (world->GetBlock(x2, world->groundHeight(x2, z2) + 1, z2) == 0 &&
            world->GetBlock(x2, world->groundHeight(x2, z2), z2) == 1) {
...
void
WorldGenerator::initOre(int WORLD_SIZE, CraftWorld *world, int oreID, float oreConcentration, int oreLowestBoundary,
                        int oreHighestBoundary, int variations) // brief and sweet function
{
    ...
    switch (oreID) {
        case 41:
            world->mainWorldVariables.worldIronOre = oreAmount;
        case 42:
            world->mainWorldVariables.worldCoalOre = oreAmount;
        case 43:
            world->mainWorldVariables.worldDiamondOre = oreAmount;
        case 44:
            world->mainWorldVariables.worldRedstoneOre = oreAmount;
        case 45:
            world->mainWorldVariables.worldRedstoneOre = oreAmount;
        case 5:
            world->mainWorldVariables.worldGoldenOre = oreAmount;
    }
