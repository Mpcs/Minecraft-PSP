...
ItemFrameEntity::ItemFrameEntity(int create_x, int create_y, int create_z, unsigned int create_facing) : BlockEntity(
    ...
    if (camFrustum.BoxInFrustum(bBox) == Frustum::Intersects &&
        ...
        if (GetItem() < 250) // block
        {
            ...
            switch (GetFacing()) {
                case 1:
                    x_offset = -PIXEL * 7.5f;
                    rotate_y_offset = -PI * 0.5f;
                    break;
                case 2:
                    x_offset = PIXEL * 7.5f;
                    rotate_y_offset = -PI * 1.5f;
                    break;
                case 3:
                    z_offset = -PIXEL * 7.5f;
                    rotate_y_offset = -PI;
            ...
            if (mWorld->blockTypes[GetItem()].blockModel == 2 ||
                mWorld->blockTypes[GetItem()].blockModel == 6) // if it is fence
            {
