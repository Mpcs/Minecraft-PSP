...
int SeekNextFrameMP3(VIRTUAL_FILE *fd) {
    ...
    if (!strncmp((char *) buf, "ID3", 3) ||
        !strncmp((char *) buf, "ea3", 3)) //skip past id3v2 header, which can cause a false sync to be found
    {
        //get the real size from the syncsafe int
        size = buf[6];
        size = (size << 7) | buf[7];
...
static int osl_at3Load(const char *fileName, AT3_INFO *info) {
    ...
    if (info->handle) {
        ...
        if (info->at3_type == TYPE_ATRAC3) {
            ...
            if (info->at3_data_align ==
                0xC0)                // atract3 have 3 bitrate, 132k,105k,66k, 132k align=0x180, 105k align = 0x130, 66k align = 0xc0
