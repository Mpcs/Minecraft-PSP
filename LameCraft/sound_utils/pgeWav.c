...
static int pgeAudioLibChannelThread(int args, void *argp) {
    ...
    while (pgeAudioLibTerminate == 0) {
        ...
        if (callback) {
            ...
            pgeAudioLibOutBlocking(channel, pgeAudioLibStatus[channel].volumeLeft,
                                   pgeAudioLibStatus[channel].volumeRight, bufptr);
        } else {
...
static int pgeAudioLibInit(int priority) {
    ...
    for (i = 0; i < PGE_NUM_AUDIO_CHANNELS; i++) {
        str[14] = '0' + i;
        pgeAudioLibStatus[i].threadHandle = sceKernelCreateThread(str, (void *) &pgeAudioLibChannelThread, priority,
