...
/*
	SOURCE 1 par d�faut: M�moire
*/

int vfsMemOpen(void *param1, int param2, int type, int mode, VIRTUAL_FILE *f) {
    //Tous les modes sont support�s mais pas tr�s bien pour l'instant, ne vous amusez pas � �crire et lire en m�me temps
...
//		return 0;
    ...
    //Initialisation du bloc m�moire
    f->offset = 0;
    f->ioPtr = param1;
    f->maxSize = param2;
    return 1;
...
int vfsMemRead(void *ptr, size_t size, size_t n, VIRTUAL_FILE *f) {
    ...
    if (f->ioPtr) {
        //min => pour �viter les d�bordements
...
int vfsMemGetc(VIRTUAL_FILE *f) {
    unsigned char car;
    //Pour la s�curit�, quand m�me un cas � part pour les fichiers
    ...
    while (1) {
        ...
        for (i = offset; i < offset + blockSize; i++) {
            ...
            if (str[i] == '\r') {
                ...
                str[i] = 0;
                //ATTENTION: MODIFIE DE -blockSize + i � -blockSize + i + 1, � v�rifier!!!
...
//D�finit la liste de fichiers pour les sources bas�es sur la m�moire
        ...
        if (!memcmp(osl_virtualFileList + i, vfl, numberOfEntries * sizeof(OSL_VIRTUALFILENAME))) {
            ...
            if (osl_virtualFileListNumber - i - numberOfEntries > 0)
                memmove(osl_virtualFileList + i, osl_virtualFileList + i + numberOfEntries,
                        osl_virtualFileListNumber - i - numberOfEntries);
            osl_virtualFileListNumber -= numberOfEntries;
        }
    }
}
...
void VirtualFileInit() {
    //On enregistre les sources par d�faut
    if (VF_MEMORY < 0)
...
//Lit un fichier entier vers la m�moire
