...
//			if (sample++ <= 2)
//				memset(bufptr, 0, osl_audioVoices[channel].numSamples << 2);
            oslAudioOutBlocking(channel, osl_audioVoices[channel].sound->volumeLeft,
                                osl_audioVoices[channel].sound->volumeRight, bufptr);
            bufidx = (bufidx ? 0 : 1);
        }
    }
...
//    AudioStatus[i].volumeleft  = OSL_VOLUME_MAX;
    ...
    strcpy(str, "audiot0");
    str[6] = '0' + i;
    AudioStatus[i].threadhandle = sceKernelCreateThread(str, (SceKernelThreadEntry) & oslAudioChannelThread, 0x10,
                                                        0x10000, 0, NULL);
