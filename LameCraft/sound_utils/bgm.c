...
short *oslDecodeADMono(OSL_ADGlobals *ad, short *dst, const unsigned char *src, unsigned int len, unsigned int samples,
...
//	sample_t* ubuf = (sample_t*) buf;
    ...
    buf2 = oslDecodeADMono(ad, (short *) buf, ad->data, l << 1, 1 << (osl_audioVoices[i].divider),
...
OSL_SOUND *oslLoadSoundFileBGM(const char *filename, int stream) {
    ...
    if (bfh.format == 1) {
        //Pour l'adpcm, dataplus contient la structure d�finissant les donn�es
