...
namespace Aurora {
    namespace Math {
        ...
        bool BoundingBox::contains(Vector3 pt) {
            return ((pt.x >= min.x) && (pt.x <= max.x) && (pt.y >= min.y) && (pt.y <= max.y) && (pt.z >= min.z) &&
                    (pt.z <= max.z));
        }
        ...
        BoundingBox::PlaneIntersectionType BoundingBox::intersect(Plane &plane) {
            ...
            float num = Vector3::dot(plane.n,
                                     vector2);//((plane.n.x * vector2.x) + (plane.n.y * vector2.y)) + (plane.n.z * vector2.z);
            if ((num + plane.d) > 0.0f) {
                return Front;
            }

            num = Vector3::dot(plane.n,
                               vector);//((plane.n.x * vector.x) + (plane.n.y * vector.y)) + (plane.n.z * vector.z);
            if ((num + plane.d) < 0.0f) {
                return Back;
            }
