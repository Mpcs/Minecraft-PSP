    ...
    namespace Graphics {
        ...
        void RenderManager::Init() {
            ...
            sceGuStencilOp(GU_KEEP, GU_KEEP,
                           GU_REPLACE); // keep value on failed test (fail and zfail) and replace on pass
        ...
        void
        RenderManager::SetFontStyle(float size, unsigned int color, unsigned int shadowColor, unsigned int options) {
            if (fontType != 0) {
                if (shadowColor == 999) {
        ...
        void RenderManager::StartFrame(float a, float b, float c) {
            ...
            if (mCam != NULL) {
                ...
                ScePspFVector3 pos = {mCam->m_vPosition.x + mCam->m_vOffset.x, mCam->m_vPosition.y + mCam->m_vOffset.y,
                                      mCam->m_vPosition.z + mCam->m_vOffset.z};
            ...
            for (unsigned int i = 0; i < count; i++) {
                ...
                if (mesh->mMaterial != -1) {
                    ...
                    if (material->mipmapping) {
                        TextureManager::Instance()->SetMipMapsTextures(material->texturID, material->mipmap1,
                                                                       material->mipmap2, material->mipmap3);
                ...
                if (mesh->triangles)
                    ...
                    sceGumDrawArray(GU_TRIANGLES, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D,
                                    mesh->vertexCount, 0, mesh->meshVertices);
                else
                    sceGumDrawArray(GU_TRIANGLE_STRIP, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D,
                                    mesh->indicesCount, 0, mesh->meshVertices);
            }
        ...
        void RenderManager::TakeScreenshot(const char *filename) {
            ...
            png_set_IHDR(png_ptr, info_ptr, 480, 272, 8, PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE,
