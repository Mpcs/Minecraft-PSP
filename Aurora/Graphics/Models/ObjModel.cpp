    ...
    namespace Graphics {
                ...
                switch (buffer[0]) {
                    ...
                    case 'K': {
                        switch (buffer[1]) {
                            case 'a'://Ka - ambient
                            {
                                fscanf(fp, "%f %f %f", &newMaterial->ambient[0], &newMaterial->ambient[1],
                            ...
                            case 'd'://Kd - diffuse
                            {
                                fscanf(fp, "%f %f %f", &newMaterial->diffuse[0], &newMaterial->diffuse[1],
                                       &newMaterial->diffuse[2]);
                                newMaterial->diffuse[3] = 1.0f;
                            }
                                break;
                            case 's'://Ks - specular
                            {
                                fscanf(fp, "%f %f %f", &newMaterial->specular[0], &newMaterial->specular[1],
                                       &newMaterial->specular[2]);
            ...
            while (!feof(fp)) {
                ...
                switch (ch) {
                    ...
                    case 'f': //face info
                    {
                        ...
                        if (sscanf(buff, "%d/%d/%d %d/%d/%d %d/%d/%d", &nFace.x, &nUVFace.x, &nNorm.x, &nFace.y,
                                   &nUVFace.y, &nNorm.y, &nFace.z, &nUVFace.z, &nNorm.z) == 9) {
                            ...
                            normal = true;
                                ...
                                sscanf(buff, "%d/%d %d/%d %d/%d", &nFace.x, &nUVFace.x, &nFace.y, &nUVFace.y, &nFace.z,
                                       &nUVFace.z) == 6) {
                            //push UV face to mesh
                            nUVFace.x -= 1;
                            nUVFace.y -= 1;
        ...
        void ObjModel::Optimize() {
            for (unsigned int i = 0; i < mMeshes.size(); i++) {
                ...
                //build mesh vertices
                mesh->meshVertices = (TexturesPSPVertex *) memalign(16, (mesh->mFace.size() * 3) *
                ...
                sceKernelDcacheWritebackInvalidateRange(mesh->meshVertices,
                                                        (mesh->mFace.size() * 3) * sizeof(TexturesPSPVertex));
            ...
            for (unsigned int i = 0; i < mMeshes.size(); i++) {
                ...
                if (mesh->mMaterial != -1) {
                    ...
                    if (mMaterials[mesh->mMaterial]->lightmapping) {
                        ...
                        size_t found = pathName.find_last_of("/");
                        pathName = pathName.substr(0, found) + "/" + mesh->name +
                                   "VRayTotalLightingMap.png";//"LightingMap.png";
            ...
            for (unsigned int i = 0; i < mMeshes.size(); i++) {
                ...
                if (ray.hasIntersected(mMeshes[i]->aabb)) {
                    ...
                    for (unsigned int j = 0; j < mMeshes[i]->mFace.size(); j++) {
                        ...
                        Vector3 vTriangle[3] = {pointX, pointY, pointZ};
                        Vector3 vLine[2] = {Vector3((ray.direction.x * rayLenght) + ray.origin.x,
                                                    (ray.direction.y * rayLenght) + ray.origin.y,
                                                    (ray.direction.z * rayLenght) + ray.origin.z), ray.origin};
