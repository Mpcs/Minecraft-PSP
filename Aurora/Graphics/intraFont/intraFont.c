...
int intraFontGetBMP(intraFont *font, unsigned short id, unsigned char glyphtype) {
    ...
    if (glyph->width > 0 && glyph->height > 0) {
        if (!(glyph->flags & PGF_BMP_H_ROWS) !=
            !(glyph->flags & PGF_BMP_V_ROWS)) { //H_ROWS xor V_ROWS (real glyph, no overlay)
                        ...
                        if (glyphtype & PGF_CHARGLYPH) {
                            ...
                            if ((font->texX + (7 - (xx & 7) + (xx & 248))) & 1) {
                                font->texture[((font->texX + (7 - (xx & 7) + (xx & 248))) +
                                               (font->texY + yy) * font->texWidth) >> 1] &= 0x0F;
                                font->texture[((font->texX + (7 - (xx & 7) + (xx & 248))) +
                                               (font->texY + yy) * font->texWidth) >> 1] |= (value << 4);
                            } else {
                                font->texture[((font->texX + (7 - (xx & 7) + (xx & 248))) +
                                               (font->texY + yy) * font->texWidth) >> 1] &= 0xF0;
                                font->texture[((font->texX + (7 - (xx & 7) + (xx & 248))) +
                                               (font->texY + yy) * font->texWidth) >> 1] |= (value);
                            }
            ...
            for (i = font->texY - 1; i < (font->texY + glyph->height + 1); i++) {
                ...
                font->texture[((font->texX + glyph->width) + (i * font->texWidth)) >> 1] &= ((font->texX +
                                                                                              glyph->width) & 1) ? 0x0F
                                                                                                                 : 0xF0;
            }
            ...
            for (i = 0; i < font->n_shadows; i++) {
                if ((font->shadowGlyph[i].flags & PGF_CACHED) && (font->shadowGlyph[i].y == glyph->y)) {
                    if ((font->shadowGlyph[i].x + font->shadowGlyph[i].width + 1) > glyph->x &&
...
int intraFontGetGlyph(unsigned char *data, unsigned long *b, unsigned char glyphtype, signed long *advancemap,
                      Glyph *glyph) {
    if (glyphtype & PGF_CHARGLYPH) {
    ...
    if (glyph->flags & PGF_CHARGLYPH) {
        ...
        (*b) += 24 + ((glyph->flags & PGF_NO_EXTRA1) ? 0 : 56) + ((glyph->flags & PGF_NO_EXTRA2) ? 0 : 56) +
...
intraFont *intraFontLoad(const char *filename, unsigned int options) {
    ...
    static const unsigned short bw_charmap_compr[] = {0x00a4, 1, 0x00a7, 2, 0x00b0, 2, 0x00b7, 1, 0x00d7, 1, 0x00e0, 2,
                                                      0x00e8, 3, 0x00ec, 2,
                                                      0x00f2, 2, 0x00f7, 1, 0x00f9, 2, 0x00fc, 1, 0x0101, 1, 0x0113, 1,
                                                      0x011b, 1, 0x012b, 1,
                                                      0x0144, 1, 0x0148, 1, 0x014d, 1, 0x016b, 1, 0x01ce, 1, 0x01d0, 1,
                                                      0x01d2, 1, 0x01d4, 1,
                                                      0x01d6, 1, 0x01d8, 1, 0x01da, 1, 0x01dc, 1, 0x0251, 1, 0x0261, 1,
                                                      0x02c7, 1, 0x02c9, 3,
                                                      0x02d9, 1, 0x0391, 17, 0x03a3, 7, 0x03b1, 17, 0x03c3, 7, 0x0401,
                                                      1, 0x0410, 64, 0x0451, 1,
                                                      0x2010, 1, 0x2013, 4, 0x2018, 2, 0x201c, 2, 0x2025, 2, 0x2030, 1,
                                                      0x2032, 2, 0x2035, 1,
                                                      0x203b, 1, 0x20ac, 1, 0x2103, 1, 0x2105, 1, 0x2109, 1, 0x2116, 1,
                                                      0x2121, 1, 0x2160, 12,
                                                      0x2170, 10, 0x2190, 4, 0x2196, 4, 0x2208, 1, 0x220f, 1, 0x2211, 1,
                                                      0x2215, 1, 0x221a, 1,
                                                      0x221d, 4, 0x2223, 1, 0x2225, 1, 0x2227, 5, 0x222e, 1, 0x2234, 4,
                                                      0x223d, 1, 0x2248, 1,
                                                      0x224c, 1, 0x2252, 1, 0x2260, 2, 0x2264, 4, 0x226e, 2, 0x2295, 1,
                                                      0x2299, 1, 0x22a5, 1,
                                                      0x22bf, 1, 0x2312, 1, 0x2460, 10, 0x2474, 40, 0x2500, 76, 0x2550,
                                                      36, 0x2581, 15, 0x2593, 3,
                                                      0x25a0, 2, 0x25b2, 2, 0x25bc, 2, 0x25c6, 2, 0x25cb, 1, 0x25ce, 2,
                                                      0x25e2, 4, 0x2605, 2,
                                                      0x2609, 1, 0x2640, 1, 0x2642, 1, 0x2e81, 1, 0x2e84, 1, 0x2e88, 1,
                                                      0x2e8b, 2, 0x2e97, 1,
                                                      0x2ea7, 1, 0x2eaa, 1, 0x2eae, 1, 0x2eb3, 1, 0x2eb6, 2, 0x2ebb, 1,
                                                      0x2eca, 1, 0x2ff0, 12,
                                                      0x3000, 4, 0x3005, 19, 0x301d, 2, 0x3021, 9, 0x303e, 1, 0x3041,
                                                      83, 0x309b, 4, 0x30a1, 86,
                                                      0x30fc, 3, 0x3105, 37, 0x3220, 10, 0x3231, 1, 0x32a3, 1, 0x338e,
                                                      2, 0x339c, 3, 0x33a1, 1,
                                                      0x33c4, 1, 0x33ce, 1, 0x33d1, 2, 0x33d5, 1, 0x3400, 6582, 0x4e00,
                                                      20902, 0xe78d, 10, 0xe7c7, 2,
                                                      0xe816, 3, 0xe81e, 1, 0xe826, 1, 0xe82b, 2, 0xe831, 2, 0xe83b, 1,
                                                      0xe843, 1, 0xe854, 2,
                                                      0xe864, 1, 0xf92c, 1, 0xf979, 1, 0xf995, 1, 0xf9e7, 1, 0xf9f1, 1,
                                                      0xfa0c, 4, 0xfa11, 1,
                                                      0xfa13, 2, 0xfa18, 1, 0xfa1f, 3, 0xfa23, 2, 0xfa27, 3, 0xfe30, 2,
                                                      0xfe33, 18, 0xfe49, 10,
                                                      0xfe54, 4, 0xfe59, 14, 0xfe68, 4, 0xff01, 94, 0xffe0, 6};
    static const unsigned char bw_shadow[] = {0x10, 0x11, 0x11, 0x01, 0x10, 0x22, 0x22, 0x01, 0x21, 0x43, 0x34, 0x12,
                                              0x31, 0x75, 0x57, 0x13,
                                              0x31, 0x86, 0x68, 0x13, 0x31, 0x86, 0x68, 0x13, 0x31, 0x75, 0x57, 0x13,
                                              0x21, 0x43, 0x34, 0x12,
    ...
    if ((strncmp(header.pgf_id, "PGF0", 4) == 0) && (header.version == 6) &&
        ...
        (header.charmap_len <= 65535) && (header.charptr_len <= 65535) && (header.charmap_bpe <= 32) &&
        (header.charptr_bpe <= 32) &&
    ...
    if (font->fileType == FILETYPE_PGF) {
        ...
        for (i = 0; i < font->n_chars; i++) {
            ...
            if (!(font->glyph[i].flags & PGF_BMP_H_ROWS) !=
                !(font->glyph[i].flags & PGF_BMP_V_ROWS)) { //H_ROWS xor V_ROWS (real glyph, not overlay)
        ...
        for (i = 0; i < font->n_chars; i++) {
            ...
            if (char_id < font->n_chars &&
                font->shadowGlyph[shadow_id].shadowID == 0) { //valid char and shadow glyph not yet loaded
                ...
                if (!(font->shadowGlyph[shadow_id].flags & PGF_BMP_H_ROWS) != !(font->shadowGlyph[shadow_id].flags &
        ...
        sceKernelDcacheWritebackAll();
        if ((options & INTRAFONT_CACHE_ASCII) &&
            ((y + ysize + 1) <= font->texHeight)) { //cache all and does it fit into cache?
...
void
intraFontSetStyle(intraFont *font, float size, unsigned int color, unsigned int shadowColor, unsigned int options) {
    if (!font) return;
    font->size = size;
    font->color = color;
...
#define LOCAL_BUFFER_LENGTH 256
    ...
    if (column >= 0) {
        ...
        x = intraFontMeasureTextUCS2Ex(font, ucs2_text,
                                       length);  //(hack to share local buffer between intraFontPrint and intraFontMeasure)
    }
...
float intraFontPrintColumnUCS2Ex(intraFont *font, float x, float y, float column, const cccUCS2 *text, int length) {
            ...
            if (char_id < font->n_chars) {
                if (font->fileType == FILETYPE_PGF) { //PGF-file
                    if ((font->glyph[char_id].flags & PGF_BMP_OVERLAY) == PGF_BMP_OVERLAY) { //overlay glyph?
                        for (j = 0; j < 3; j++) {
                            subucs2 = font->fontdata[(font->glyph[char_id].ptr) + j * 2] +
    ...
    for (i = 0; i < length; i++) {
                ...
                if (font->options & INTRAFONT_SCROLL_LEFT) { //scrolling (any direction) requested
                        ...
                        switch (font->options & PGF_SCROLL_MASK) {
                            ...
                            case INTRAFONT_SCROLL_LEFT:                                  //scroll left
                                ...
                                if (count < 60) {
                                    ...
                                    left -= (count -
                                             60); //scroll left for (textwidth/60) s (speed = 1 pix/frame) and disappear for 0.5s
                                } else if (count < (textwidth + 120)) {
                                    color = (color & 0x00FFFFFF) |
                                            ((((color >> 24) * (count - textwidth - 90)) / 30) << 24); //fade-in in 0.5s
                                    shadowColor = (shadowColor & 0x00FFFFFF) |
                                                  ((((shadowColor >> 24) * (count - textwidth - 90)) / 30) << 24);
                                } else {
                            ...
                            case INTRAFONT_SCROLL_RIGHT:  //scroll right
                                ...
                                if (count < 60) {
                                    ...
                                    left -= textwidth - count +
                                            60; //scroll right for (textwidth/60) s (speed = 1 pix/frame) and disappear for 0.5s
                                } else if (count < (textwidth + 120)) {
                                    left -= textwidth;
                                    color = (color & 0x00FFFFFF) |
                                            ((((color >> 24) * (count - textwidth - 90)) / 30) << 24); //fade-in in 0.5s
                    ...
                    for (j = i; j < length; j++) {
                        ...
                        if (intraFontMeasureTextUCS2Ex(font, text + i, j + 1 - i) >
                            ...
                            if (n_spaces > 0)
                                fill = (column - intraFontMeasureTextUCS2Ex(font, text + i, eol - i)) /
                                       ((float) n_spaces);
                            break;
                        }
                    }
        ...
        if (char_id < font->n_chars) {
            ...
            glyph_ptr = (font->fileType == FILETYPE_PGF) ? char_id
            ...
            if (font->options & INTRAFONT_WIDTH_FIX) {
                width += (((float) (font->options & PGF_WIDTH_MASK)) / 2.0f -
                          ((float) font->glyph[glyph_ptr].advance) / 8.0f) * glyphscale;
            }
            ...
            if (c_index > last_c_index) {
                ...
                s1->c = shadowColor;
                s1->x = left + width +
                        (shadowGlyph->width + shadowGlyph->left) * glyphscale * 64.0f / ((float) font->shadowscale);
                s1->y = top + height +
                        (shadowGlyph->height - shadowGlyph->top) * glyphscale * 64.0f / ((float) font->shadowscale);
                s1->z = 0.0f;
            ...
            if (font->altFont) {
                ...
                width += intraFontPrintColumnUCS2Ex(font->altFont, left + width, top + height, 0.0f, text + i, 1) -
                         (left + width);
...
float intraFontMeasureTextEx(intraFont *font, const char *text, int length) {
    return intraFontPrintColumnEx(font, 0.0f, 0.0f, -1.0f, text,
                                  length); //explanation: intraFontPrintColumnEx does the String -> UCS2 conversation,
...
float intraFontMeasureTextUCS2Ex(intraFont *font, const cccUCS2 *text, int length) {
    ...
    for (i = 0; (i < length) && (text[i] != '\n'); i++) { //measure until end of string or first newline char
        ...
        if (char_id < font->n_chars) {
            unsigned short glyph_ptr = (font->fileType == FILETYPE_PGF) ? char_id
                                                                        : 0; //for fields not covered in GlyphBW (advance,...)
            x += (font->options & INTRAFONT_WIDTH_FIX) ? (font->options & PGF_WIDTH_MASK) * font->size :
                 (font->glyph[glyph_ptr].advance) * font->size * 0.25f;
        } else {
            x += intraFontMeasureTextUCS2Ex(font->altFont, text + i,
