...
void
WeatherSystem::Render(CraftWorld *mWorld, Frustum &camFrustum, float camera_angle, Vector3 playerPosition, float dt) {
    raindropsCount = 0;
    if (GetWeatherType() == RAIN && (mWorld->desertBiomeRadius == -1 ||
                                     mWorld->FastDistance2d(abs(playerPosition.x - mWorld->desertBiomePosition.x),
                                                            abs(playerPosition.z - mWorld->desertBiomePosition.z)) >
                                     mWorld->desertBiomeRadius)) {
        bool raindrop = false;
        ...
        if (mWorld->snowBiomeRadius != -1 &&
            (mWorld->FastDistance2d(abs(playerPosition.x - mWorld->snowBiomePosition.x),
                                    abs(playerPosition.z - mWorld->snowBiomePosition.z)) <= mWorld->snowBiomeRadius)) {
            raindrop = itIsRaining = false;
            snowflake = itIsSnowing = true;

            TextureManager::Instance()->SetTextureModeulate(
                    TextureHelper::Instance()->GetTexture(TextureHelper::snowTexture));
        } else {
            raindrop = itIsRaining = true;
            snowflake = itIsSnowing = false;

            TextureManager::Instance()->SetTextureModeulate(
                    TextureHelper::Instance()->GetTexture(TextureHelper::rainTexture));
        }
