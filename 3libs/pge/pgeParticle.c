...
void
pgeParticleSystemSetInfoFromMemory(pgeParticleSystem *ps, const char *psi, int sizePsi, pgeTexture *spritetexture) {
...
void pgeParticleSystemUpdate(pgeParticleSystem *ps, float deltatime) {
    ...
    if (ps->age != -2.0f) {
        ...
        for (i = 0; i < nParticlesCreated; i++) {
            ...
            pgeMathVecScale3f(&par->vecVelocity, &par->vecVelocity,
                              pgeMathRandFloat(ps->info.fSpeedMin, ps->info.fSpeedMax));
            ...
            par->fSize = pgeMathRandFloat(ps->info.fSizeStart, ps->info.fSizeStart +
                                                               (ps->info.fSizeEnd - ps->info.fSizeStart) *
                                                               ps->info.fSizeVar);
            par->fSizeDelta = (ps->info.fSizeEnd - par->fSize) / par->fTerminalAge;

            par->fSpin = pgeMathRandFloat(ps->info.fSpinStart, ps->info.fSpinStart +
                                                               (ps->info.fSpinEnd - ps->info.fSpinStart) *
                                                               ps->info.fSpinVar);
            par->fSpinDelta = (ps->info.fSpinEnd - par->fSpin) / par->fTerminalAge;

            par->colColor.r = pgeMathRandFloat(ps->info.colColorStart.r, ps->info.colColorStart.r +
                                                                         (ps->info.colColorEnd.r -
                                                                          ps->info.colColorStart.r) *
                                                                         ps->info.fColorVar);
            par->colColor.g = pgeMathRandFloat(ps->info.colColorStart.g, ps->info.colColorStart.g +
                                                                         (ps->info.colColorEnd.g -
                                                                          ps->info.colColorStart.g) *
                                                                         ps->info.fColorVar);
            par->colColor.b = pgeMathRandFloat(ps->info.colColorStart.b, ps->info.colColorStart.b +
                                                                         (ps->info.colColorEnd.b -
                                                                          ps->info.colColorStart.b) *
                                                                         ps->info.fColorVar);
            par->colColor.a = pgeMathRandFloat(ps->info.colColorStart.a, ps->info.colColorStart.a +
                                                                         (ps->info.colColorEnd.a -
                                                                          ps->info.colColorStart.a) *
                                                                         ps->info.fAlphaVar);
