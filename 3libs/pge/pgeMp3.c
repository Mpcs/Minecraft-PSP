    ...
    while (!pgeMp3ThreadExit) {
        ...
        while (pgeMp3JobStarted) {
            ...
            if (!pgeMp3Paused) {
                ...
                if (((outbits & PGE_MP3_BUFFER1_EMPTY) == 0 && pgeMp3WorkingBuffer == 0) ||
                    ((outbits & PGE_MP3_BUFFER2_EMPTY) == 0 && pgeMp3WorkingBuffer == 1) ||
                    ((outbits & PGE_MP3_BUFFER3_EMPTY) == 0 && pgeMp3WorkingBuffer == 2))
                    sceKernelDelayThreadCB(1);
                else
                    continue;

                sceAudioSRCOutputBlocking(pgeMp3VolumeValue,
                                          &pgeMp3SrcBuffer[pgeMp3WorkingBuffer][pgeMp3DecodeCurrentFrame *
                                                                                pgeMp3SamplesPerFrame * 2]);
...
static int pgeMp3Init(void) {
    ...
    pgeMp3ReadThreadId = sceKernelCreateThread("PgeMp3ReadThread", pgeMp3ReadThread, 14, 0x2000, PSP_THREAD_ATTR_USER,
                                               0);
    ...
    pgeMp3DecodeThreadId = sceKernelCreateThread("PgeMp3DecodeThread", pgeMp3DecodeThread, 15, 0x2000,
