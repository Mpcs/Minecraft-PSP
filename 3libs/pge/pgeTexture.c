...
pgeTexture *
pgeTextureCreate(unsigned int width, unsigned int height, enum pgePixelFormat format, enum pgeMemoryLocation location) {
...
pgeTexture *
pgeTextureLoadMemory(unsigned char *buffer, int size, enum pgeMemoryLocation location, unsigned char swizzle) {
    pgeTexture *texture = (pgeTexture *) pgeMalloc(sizeof(pgeTexture));
...
static void
pgeTexturePaletteGet(pgeTexture *texture, unsigned int col, unsigned int *r, unsigned int *g, unsigned int *b,
                     unsigned int *a) {
    if (texture == 0)
        return;
...
void pgeTextureSave(pgeTexture *texture, const char *filename) {
    ...
    png_set_IHDR(png_ptr, info_ptr, texture->width, texture->height, 8, col_type, PNG_INTERLACE_NONE,
...
void pgeTextureScreenshot(const char *filename) {
    ...
    png_init_io(png_ptr, fp);
    png_set_IHDR(png_ptr, info_ptr, 480, 272, 8, col_type, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT,
