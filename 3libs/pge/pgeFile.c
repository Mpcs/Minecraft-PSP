...
int pgeFileReadFile(pgeFile *s, void *buf, unsigned int size) {
    ...
    if (((pgeFileStream *) s->_stream)->fsize ==
        (unsigned int) sceIoLseek32(((pgeFileStream *) s->_stream)->fd, 0, PGE_FILE_CUR))
        s->_stream->eos = 1;
...
int pgeFileRead(pgeFile *s, void *buf, unsigned int size) {
    if (s == 0 || s->_stream == 0 || s->_stream->read == 0 ||
        (s->_stream->mode & (PGE_FILE_RDONLY | PGE_FILE_RDWR)) == 0)
        return -1;
...
int pgeFileWrite(pgeFile *s, void *buf, unsigned int size) {
    if (s == 0 || s->_stream == 0 || s->_stream->write == 0 ||
        (s->_stream->mode & (PGE_FILE_WRONLY | PGE_FILE_RDWR)) == 0)
        return -1;
