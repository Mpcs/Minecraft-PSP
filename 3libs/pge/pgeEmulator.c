...
const int emu_pixelPhysFormats[] = {GL_UNSIGNED_SHORT_5_6_5_REV, GL_UNSIGNED_SHORT_1_5_5_5_REV,
                                    GL_UNSIGNED_SHORT_4_4_4_4_REV, GL_UNSIGNED_BYTE, 0, 0};
...
SceUID
sceKernelCreateThread(const char *name, SceKernelThreadEntry entry, int initPriority, int stackSize, SceUInt attr,
...
void sceGuClearColor(unsigned int color) {
    glClearColor((GLclampf)((color & 0xff) / 255.0f), (GLclampf)(((color >> 8) & 0xff) / 255.0f),
                 (GLclampf)(((color >> 16) & 0xff) / 255.0f), (GLclampf) 255.0f);
}
...
void emuSetVertexColor(unsigned int color) {
    ...
    if (emuCurrentTextureState == 0)
        col = RGBA(((color & 255) * (col & 255)) / 255, (((color >> 8) & 255) * ((col >> 8) & 255)) / 255,
                   (((color >> 16) & 255) * ((col >> 16) & 255)) / 255,
                   (((color >> 24) & 255) * ((col >> 24) & 255)) / 255);
...
void sceGuDrawArray(int prim, int vtype, int count, const void *indices, const void *vertices) {
    ...
    if (prim == GU_SPRITES) {
        while (count > 0) {
            ...
            glTexCoord2f(v[0].u, v[0].v);
            glVertex3f(v[0].x, v[0].y, 0); //1
            glTexCoord2f(v[0].u, v[1].v);
            glVertex3f(v[0].x, v[1].y, 0); //4
            glTexCoord2f(v[1].u, v[1].v);
            glVertex3f(v[1].x, v[1].y, 0); //3
            glTexCoord2f(v[1].u, v[0].v);
            glVertex3f(v[1].x, v[0].y, 0); //2
            glEnd();
        ...
        while (count > 0) {
            ...
            glTexCoord2f(v[2].u, v[2].v);
            glVertex3f(v[2].x, v[2].y, v[2].z);
            if (v[3].colored)
                emuSetVertexColor(v[3].color);
            glTexCoord2f(v[3].u, v[3].v);
...
int sceNetAdhocPtpOpen(unsigned char *srcmac, unsigned short srcport, unsigned char *destmac, unsigned short destport,
                       unsigned int bufsize, unsigned int delay, int count, int unk1) {
    return 0;
}
...
int
sceNetAdhocPtpListen(unsigned char *srcmac, unsigned short srcport, unsigned int bufsize, unsigned int delay, int count,
                     int queue, int unk2) {
    return 0;
}
...
int sceNetAdhocMatchingCreate(int mode, int maxpeers, unsigned short port, int bufsize, unsigned int hellodelay,
                              unsigned int pingdelay, int initcount, unsigned int msgdelay,
...
int sceNetAdhocMatchingStart(int matchingid, int evthpri, int evthstack, int inthpri, int inthstack, int optlen,
                             void *optdata) {
    return 0;
}
...
int sceNetAdhocctlCreateEnterGameMode(const char *name, int unknown, int num, unsigned char *macs, unsigned int timeout,
                                      int unknown2) {
    return 0;
}
