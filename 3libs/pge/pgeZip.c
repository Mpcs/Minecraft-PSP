...
static unsigned long pgeZipLocateCentralDir(FILE *file) {
    ...
    while (ubackread < umaxback) {
        ...
        ureadsize = ((PGE_ZIP_BUF_READ_COMMENT + 4) < (usizefile - ureadpos)) ? (PGE_ZIP_BUF_READ_COMMENT + 4) : (
                usizefile - ureadpos);
...
static int pgeZipGetZipFileInfoInternal(pgeZip *file, zipFileInfo *pfileinfo, zipFileInternalInfo *pfileinfointernal,
...
static int pgeZipGotoFirstFile(pgeZip *file) {
    ...
    s->numfile = 0;
    err = pgeZipGetZipFileInfoInternal(file, &s->currentfileinfo, &s->currentfileinfointernal, NULL, 0, NULL, 0, NULL,
                                       0);
...
static int
pgeZipGetCurrentFileInfo(pgeZip *file, zipFileInfo *pfileinfo, char *filename, unsigned long filenamebuffersize,
                         void *extrafield, unsigned long extrafieldbuffersize, char *comment,
                         unsigned long commentbuffersize) {
    return pgeZipGetZipFileInfoInternal(file, pfileinfo, NULL, filename, filenamebuffersize, extrafield,
...
static int pgeZipGotoNextFile(pgeZip *file) {
    ...
    s->posincentraldir +=
            PGE_ZIP_CENTRALDIR_ITEM_SIZE + s->currentfileinfo.filenamesize + s->currentfileinfo.fileextrasize +
            s->currentfileinfo.filecommentsize;
    s->numfile++;
...
static int
pgeZipCheckCurrentFileCoherencyHeader(_zip *s, unsigned int *pisizevar, unsigned long *poffsetstaticextrafield,
    ...
    *psizestaticextrafield = 0;
    ...
    if ((err == PGE_ZIP_OK) && (s->currentfileinfo.compressionmethod != 0) &&
...
static int pgeZipOpenCurrentFile(pgeZip *file, const char *password) {
    ...
    if (password != NULL) {
        ...
        if (fseek(pfileinzipreadinfo->file, s->currentzipfileinfo->posinzip + s->currentzipfileinfo->bytebeforezip,
                  SEEK_SET) != 0) {
            pgeFree(pfileinzipreadinfo->buffer);
            pgeFree(pfileinzipreadinfo);
...
static int pgeZipReadCurrentFile(pgeZip *file, void *buf, unsigned int len) {
    ...
    while (pfileinzipreadinfo->stream.avail_out > 0) {
        if ((pfileinzipreadinfo->stream.avail_in == 0) && (pfileinzipreadinfo->restreadcompressed > 0)) {
            ...
            if (fseek(pfileinzipreadinfo->file, pfileinzipreadinfo->posinzip + pfileinzipreadinfo->bytebeforezip,
                      SEEK_SET) != 0)
                return PGE_ZIP_ERRNO;
