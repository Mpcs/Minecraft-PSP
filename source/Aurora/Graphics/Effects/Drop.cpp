...
Drop::Drop(Vector3 position2, int id2, int amount2, bool stackable2, int num2) {
    ...
    bBox = BoundingBox(Vector3(position.x - 0.25f, position.y - 0.25f, position.z - 0.25f),
                       Vector3(position.x + 0.25f, position.y + 0.25f, position.z + 0.25f));
    ...
    if (onGround == true && checkForOtherDrops == false) {
        for (int i = 0; i < myCrft->mDrops.size(); i++) {
            ...
            if (num != TestDrop->num) {
                ...
                if (abs(position.x - TestDrop->position.x) < distance &&
                    abs(position.y - TestDrop->position.y) < distance &&
                    abs(position.z - TestDrop->position.z) < distance) {
...
void Drop::Update(float dt) {
    ...
    if (toDestroy == false && getMe == false && active == true) {
        ...
        if (myCrft->BlockSolid(position.x + velocity.x * dt * 2, position.y, position.z + velocity.z * dt * 2) ==
            true) {
            velocity.x *= -0.9f;
            velocity.z *= -0.9f;
            /*
        ...
        if (myCrft->BlockSolid(position.x + 0.1f, position.y, position.z) == true &&
            myCrft->BlockSolid(position.x - 0.1f, position.y, position.z) == true &&
            myCrft->BlockSolid(position.x, position.y, position.z + 0.1f) == true &&
            myCrft->BlockSolid(position.x, position.y, position.z - 0.1f) == true &&
            myCrft->BlockSolid(position.x, position.y + 0.1f, position.z) == true &&
            myCrft->BlockSolid(position.x, position.y - 0.1f, position.z) == true) {
            position.y += 1.1f;
            velocity.y = 2.0f;
        }
...
void Drop::Render(Frustum &camFrustum, Vector3 playerPos, float camAngle) {
    if (abs(position.x - playerPos.x) < 10.0f && abs(position.y - playerPos.y) < 11.0f &&
        abs(position.z - playerPos.z) < 10.0f) {
        active = true;
    } else {
        active = false;
    }
    ...
    if (toDestroy == false && getMe == false && active == true) {
        ...
        if (camFrustum.BoxInFrustum(bBox) == Frustum::Intersects) {
            ...
            float angle = ATAN2_fast(position.x - (playerPos.x + (cosf(camAngle + PI) * 4)),
                                     position.z - (playerPos.z + (sinf(camAngle + PI) * 4))) - PI * 0.5f;
