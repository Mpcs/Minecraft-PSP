...
TNT::TNT(CraftWorld *world, Vector3 position2, Vector3 velocity2) {
    ...
    if (!world->BlockSolid(position2.x - 1, position2.y, position2.z) &&
        !world->BlockSolid(position2.x + 1, position2.y, position2.z)) {
        velocity.x = -0.6f + (rand() % 2) * 1.2f;
    }
    if (!world->BlockSolid(position2.x, position2.y, position2.z - 1) &&
        !world->BlockSolid(position2.x, position2.y, position2.z + 1)) {
    ...
    if (myWorld->mTNTs.size() > 1) {
        for (unsigned int j = 0; j < myWorld->mTNTs.size(); j++) {
            if (j < myWorld->mTNTs.size()) {
                ...
                if (TestTNT2 != NULL) {
                    if (TestTNT2->num != num && TestTNT2->position.x != position.x &&
                        TestTNT2->position.z != position.z) {
                        float distance = (myWorld->FastDistance2d(abs(position.x - TestTNT2->position.x) * 10,
                                                                  abs(position.z - TestTNT2->position.z) * 10)) /
                                         10.0f + abs(position.y - TestTNT2->position.y);
                        if (distance <= 4.0f && distance >= 0.0f) {
                            float angle2 = ATAN2_fast(position.x - (TestTNT2->position.x),
                                                      position.z - (TestTNT2->position.z)) + PI;

                            TestTNT2->velocity.x += sinf(angle2) * (7.5f - distance) * (7.5f - distance);
                            TestTNT2->velocity.z += cosf(angle2) * (7.5f - distance) * (7.5f - distance);
                            TestTNT2->velocity.y += ((TestTNT2->position.y - position.y) / 2.5f) * (6.5f - distance) *
                                                    (6.5f - distance);
                        }
                    }
                }
    ...
    if (myWorld->mZombies.empty() == false) {
        for (unsigned int j = 0; j < myWorld->mZombies.size(); j++) {
            if (j < myWorld->mZombies.size()) {
                ...
                if (TestMob != NULL) {
                    float dist = myWorld->FastDistance2d(abs(position.x - TestMob->position.x) * 10,
                                                         abs(position.z - TestMob->position.z) * 10) / 10.0f +
                                 abs(position.y - (TestMob->position.y - 0.7)) / 2.0f;
...
void TNT::Update(float dt) {
    if (position.x < 1 || position.x >= myWorld->WORLD_SIZE - 1 || position.y < 1 ||
        position.y >= myWorld->WORLD_HEIGHT - 1 || position.z < 1 || position.z >= myWorld->WORLD_SIZE - 1) {
        toDestroy = true;
    }
    ...
    if (myWorld->BlockSolid(position.x - 0.45f, position.y - 0.5f, position.z - 0.45f) == false &&
        ...
        myWorld->BlockSolid(position.x + 0.45f, position.y - 0.5f, position.z - 0.45f) == false &&
        myWorld->BlockSolid(position.x + 0.45f, position.y - 0.5f, position.z + 0.45f) ==
        false) // check solid blocks in 4 points
    {
    ...
    if (myWorld->BlockSolid(position.x - 0.45f, position.y - 0.2f, position.z - 0.45f) == true ||
        ...
        myWorld->BlockSolid(position.x - 0.45f, position.y + 0.2f, position.z + 0.45f) == true ||
        myWorld->BlockSolid(position.x - 0.45f, position.y + 0.2f, position.z - 0.45f) ==
        true) // check solid blocks in 4 points
    {
        if (velocity.x < 0) {
            velocity.x *= -0.65f;
        }
    ...
    if (timeToExplode <= 0.0f && toDestroy == false) // if our time has ended
    {
        ...
        float dist = myWorld->FastDistance2d(abs(position.x - myWorld->playerPos.x) * 10,
                                             abs(position.z - myWorld->playerPos.z) * 10) / 10.0f +
                     abs(position.y - (myWorld->playerPos.y - 0.7)) / 2.0f;
        ...
        if (myWorld->GetBlock(position.x, position.y, position.z) != WaterBlock::getID() &&
            myWorld->GetBlock(position.x, position.y, position.z) != Lava::getID()) {
            myWorld->BuildExplodeSphere(explodeRadius, position.x, position.y,
                                        position.z); // destroy blocks in radius of 3 meters
...
void TNT::Render() {
    ...
    float myLight =
            myWorld->BlockFinalLight(position.x, position.y, position.z) * 1.35f; // fetching the light of position
    ...
    (int) animationTime % 2 == 0 ? myWorld->drawCubes(TNTid, myLight) : myWorld->drawWhiteCube(TNTid,
                                                                                               myLight); // our animation magic
