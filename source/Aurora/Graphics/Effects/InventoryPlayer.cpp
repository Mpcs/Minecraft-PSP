...
namespace Aurora {
    namespace Graphics {
        ...
        void InventoryPlayer::Render(CraftWorld *world, float dt, float angle, int texNum, int handItemId) {
            ...
            if (handItemId != -1) {
                if (handItemId >= 250) {
                    world->ItemHaveTerrainTexture(handItemId) == true ? TextureManager::Instance()->SetTextureModeulate(
                            world->textureTerrainId) : TextureManager::Instance()->SetTextureModeulate(
                            world->textureItemsId);
            ...
            if (world->armorId[0] != 324) {
                ...
                if (world->armorId[0] == Pumpkin3::getID()) {
                    ...
                    TextureManager::Instance()->SetTextureModeulate(
                            TextureHelper::Instance()->GetTexture(TextureHelper::Terrain1));
