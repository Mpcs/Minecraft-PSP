    ...
    namespace Graphics {
        Particle::Particle(CraftWorld *world, std::string _name, Vector3 _position) {
            ...
            if (name == "smoke") {
                ...
                switch (rand() % 8) {
                    case 0 :
                        SetLiveTime(1500, 2000);
                        break;
                    case 1 :
                        SetLiveTime(2500, 3000);
                        break;
                    case 2 :
                        SetLiveTime(2500, 3000);
                        break;
                    case 3 :
                        SetLiveTime(3000, 3500);
                        break;
                    case 4 :
                        SetLiveTime(3000, 3500);
                        break;
                    default:
                        SetLiveTime(2000, 2500);
                        break;
                }
            ...
            bBox = BoundingBox(Vector3(position.x - 0.33f, position.y - 0.33f, position.z - 0.33f),
                               Vector3(position.x + 0.33f, position.y + 0.33f, position.z + 0.33f));
        ...
        void Particle::BuildVerticiesForBlockCrack(CraftWorld *crtf, int blockId) {
            ...
            int tex_x, tex_y;
            if (crtf->mainOptions.fastRendering == true &&
                (blockId == LeavesBlock::getID() || blockId == BirchLeaves::getID() ||
                 blockId == SpruceLeaves::getID())) {
        ...
        void Particle::Update(CraftWorld *crtf, float dt) {
            if (position.x < 0 || position.x >= crtf->WORLD_SIZE || position.y < 0 ||
                position.y >= crtf->WORLD_HEIGHT || position.z < 0 || position.z >= crtf->WORLD_SIZE) {
                toDestroy = true;
            }
            ...
            if (name == "block crack" || name == "item crack") {
                ...
                if (crtf->BlockSolid(position.x + velocity.x * dt * 2, position.y, position.z + velocity.z * dt * 2) ==
                    true) {
                    velocity.x *= -0.9f;
                    velocity.z *= -0.9f;
                }
        ...
        void Particle::Render(Frustum &camFrustum) {
            if (rederMe == true && toDestroy == false && particleVertices != NULL &&
                ...
                sceGumDrawArray(GU_TRIANGLE_STRIP, GU_TEXTURE_32BITF | GU_VERTEX_32BITF | GU_TRANSFORM_3D, 4, 0,
                                particleVertices);
