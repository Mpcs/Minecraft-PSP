...
char
cccLZRNextBit(unsigned char *buf_ptr1, int *number, unsigned int *test_mask, unsigned int *mask, unsigned int *buffer,
...
int cccLZRGetNumber(signed char n_bits, unsigned char *buf_ptr, char inc, char *flag, unsigned int *mask,
    ...
    *next_in = in + 5;
    ...
    while (1) {
        ...
        if (!cccLZRNextBit(buf_ptr1, 0, &mask, &mask, &buffer, next_in)) {
            ...
            buf_ptr1 = buf +
                       (((((((int) (next_out - (unsigned char *) out)) & 0x07) << 8) + last_char) >> type) & 0x07) *
                       0xFF - 0x01;
            for (j = 1; j <= 0xFF;) {
            ...
            *next_out++ = j;
            ...
            if ((flag != 0) || (n_bits >= 0)) {
                buf_ptr1 =
                        buf + (n_bits << 5) + (((((int) (next_out - (unsigned char *) out)) << n_bits) & 0x03) << 3) +
                        buf_off + 2552;
        ...
        while (str[i] && length < count) {
            ...
            for (j = 1; (j <= header[2]) && (id < 0); j++) {
                if ((code >= header[j * 3]) && (code <= header[j * 3 + 1])) {
                    ...
                    if (j == 2)
                        code = 0x0200 * str[i] - 0xE100 - ((str[i] >= 0xE0) ? 0x8000 : 0) + str[i + 1] +
                               ((str[i + 1] <= 0x7E) ? -0x1F : ((str[i + 1] >= 0x9F) ? 0x82 : -0x20));
                }
            }
            dst[length++] = (id < 0) ? __error_char_ucs2__ : SJIStoUCS2[id];
            i += (str[i] <= 0x80 || (str[i] >= 0xA0 && str[i] <= 0xDF) || str[i] >= 0xFD) ? 1
        ...
        if (str[i] <= 0x7f) {
            ...
            if (__table_ptr__[CCC_CP936]) { //table is present
                ...
                for (entry = (unsigned char *) (__table_ptr__[CCC_CP936]);
                     ...
                     ((entry[0] + 0x100 * entry[1] + entry[4]) <= code); entry += 5);
        ...
        if (str[i] <= 0x7f) {
            ...
            if (__table_ptr__[CCC_CP950]) { //table is present
                ...
                for (entry = (cccTblEntry *) (__table_ptr__[CCC_CP950]);
                     (entry < (cccTblEntry *) (__table_end__[CCC_CP950])) && (entry->code < code); entry++);
    ...
    switch (cp) {
        ...
        default:
            if (cp < CCC_N_CP) { //codepage in range?
                ...
                while (str[length] && length <
                                      count) { //conversion: ASCII (if ASCII) or LUT-value (if LUT exists) or error_char (if LUT doesn't exist)
                    if (str[length] < 0x80) {
                        dst[length] = (cccUCS2) str[length];
                    } else {
                        dst[length] = ((__table_ptr__[cp]) ? ((cccUCS2 * )(__table_ptr__[cp]))[str[length] - 0x80]
                                                           : __error_char_ucs2__);
