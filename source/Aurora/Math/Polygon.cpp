...
namespace Aurora {
    namespace Math {
        ...
        bool Polygon::IntersectedPlane(Vector3 vPoly[], Vector3 vLine[], Vector3 &vNormal, float &originDistance) {
            ...
            vNormal = Vector3::Normal(
                    vPoly);                            // We need to get the normal of our plane to go any further
            ...
            if (distance1 * distance2 >=
                0)            // Check to see if both point's distances are both negative or both positive
        ...
        Vector3 Polygon::IntersectionPoint(Vector3 vNormal, Vector3 vLine[], double distance) {
            ...
            Denominator = Vector3::dot(vNormal,
                                       vLineDir);        // Get the dot product of the line's vector and the normal of the plane
        ...
        bool Polygon::InsidePolygon(Vector3 vIntersection, Vector3 Poly[], long verticeCount) {
            ...
            {
                vA = Vector3::fromVectors(Poly[i],
                                          vIntersection);    // Subtract the intersection point from the current vertex
                // Subtract the point from the next vertex
                vB = Vector3::fromVectors(Poly[(i + 1) % verticeCount], vIntersection);

                Angle += Polygon::AngleBetweenVectors(vA,
                                                      vB);    // Find the angle between the 2 vectors and add them all up as we go along
            }
        ...
        bool Polygon::IntersectedPolygon(Vector3 vPoly[], Vector3 vLine[], int verticeCount, Vector3 &vIntersection,
                                         Vector3 &vNormal) {
            //Vector3 vNormal;
