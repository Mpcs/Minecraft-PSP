...
namespace noisepp {
    ...
    public:
        TranslatePointElement3D(const Pipeline3D *pipe, ElementID element, Real TranslationX, Real TranslationY,
                                Real TranslationZ) :
                mElement(element), mTranslationX(TranslationX), mTranslationY(TranslationY),
        ...
        virtual Real getValue(Real x, Real y, Real z, Cache *cache) const {
            return getElementValue(mElementPtr, mElement, x + mTranslationX, y + mTranslationY, z + mTranslationZ,
                                   cache);
        }
    ...
    */
    ...
    public:
        ...
        virtual ElementID addToPipeline(Pipeline3D *pipe) const {
            //NoiseModuleCheckSourceModules;
            ElementID first = getSourceModule(0)->addToPipeline(pipe);
            return pipe->addElement(this, new TranslatePointElement3D(pipe, first, mTranslationX, mTranslationY,
                                                                      mTranslationZ));
        }
